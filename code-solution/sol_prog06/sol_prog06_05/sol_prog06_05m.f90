...
contains
    ...
    subroutine output(it, fname)
        ...
        write(fname,'(a)')'   tauw   taur   tauc   tauk   taup    pen      B'
        write(fname,'(7f7.2/)')tauw(it), taur(it), tauc(it), tauk(it), taup(it), pen(it), BB(it)
    ...
    subroutine output_summary(fname)
        ...
        ! current generations
        write(fname,'(i3,119x,2f7.2)')-1, v(-1), &
            ((util(3, 1)/util(3, 0))**(1d0/egam)-1d0)*100d0
        write(fname,'(i3,19f7.2)')0, c(1, 0), c(2, 0), c(3, 0), AA(0), KK(0), r(0), w(0), &
            YY(0), BB(0), BA(0), BF(0), TB(0), tauc(0), tauw(0), taur(0), tauk(0), taup(0), &
            v(0), ((util(2, 1)/util(2, 0))**(1d0/egam)-1d0)*100d0

        ! future generations
        do it = 1, TT
            diff = YY(it) - CC(it) - GG(it) - II(it) - TB(it)
            write(fname,'(i3,19f7.2,f7.3)')it, c(1, it), c(2, it), c(3, it), AA(it), &
                KK(it), r(it), w(it), YY(it), BB(it), BA(it), BF(it), TB(it), tauc(it), tauw(it), &
                taur(it), tauk(it), taup(it), v(it), &
