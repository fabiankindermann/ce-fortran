...
! PROGRAM ENHCRET_OLG
!
! ##  Endogenous human capital and retirement
!
...
contains
    ...
    function get_W(ij,it)
        ...
        do ijp = ij+1, JJ
            itp = year(it, ij, ijp)
            PRn = PRn*Rn(itp)
            get_W = get_W +  wn(ijp,itp)/PRn
            if(ijp > JR) get_W = get_W + pen(ijp,itp)/PRn
        enddo
    ...
    function get_Psi(ij,it)
        ...
        do ijp = ij, JJ
            ...
            if(ijp /= JR)then
                ...
                get_Psi = get_Psi+beta**((ijp-ij)*gamma)*(p(itp)/PRn/p(it))**(1d0-gamma)*vv**((1d0-gamma)/(rho-gamma))
            endif
            PRn = PRn*Rn(itp1)
        enddo
...
! subroutine for calculating the optimal consumption path
    subroutine get_path(ij,it)
        ...
        do ijp = ij+1, JJ
            ...
            if(ij == JR)then
                ...
                c(6,it+1) = (beta*Rn(it+1)*p(it)/p(it+1))**gamma/vv*c(ij,it)
            endif

            if(ijp /= JJ)then
                a(ijp, itp) = wn(ijp-1,itm) + pen(ijp-1,itm)+ Rn(itm)*a(ijp-1,itm) - p(itm)*c(ijp-1,itm)
            else
                a(ijp, itp) = wn(ijp-1,itm)*(1d0-l(itm))+l(itm)*pen(ijp-1,itm)+Rn(itm)*a(ijp-1,itm)-p(itm)*c(ijp-1,itm)
            endif
    ...
    subroutine output(it, fname)
        ...
        write(fname,'(a)')'     LL      e     h2      l     w      mu   util'
        write(fname,'(7f7.2/)')LL(it),e(it),h(2,it),l(it),w(it),mu(it),util(1,it)
        ...
        if(fname > 1)then
            write(fname,'(a)')' Age    cons       yn      pen    asset    Diff      '
            do ij = 1, JJ
                itp = year(it, 1, 2)
                if(ij < JJ)then
                    diff = Rn(it)*a(ij,it)+wn(ij,it)+pen(ij,it)-a(ij+1,itp)-p(it)*c(ij,it)
                ...
                if(ij == 1)then
                    write(fname,'(i3,5f9.2)')ij,c(ij,it),wn(ij,it)*(1d0-(1d0-taus(it))*e(it)),pen(ij,it),a(ij,it),diff
                elseif(ij == JR)then
                    write(fname,'(i3,5f9.2)')ij,c(ij,it),wn(ij,it)*(1d0-l(it)),l(it)*pen(ij,it),a(ij,it),diff
                else
                    write(fname,'(i3,5f9.2)')ij,c(ij,it),wn(ij,it),pen(ij,it),a(ij,it),diff
                endif
            enddo
            write(fname,'(a/)')'                                           '
        endif
    ...
    subroutine output_summary(fname)
        ...
        ! current generations
        do ij = JJ, 3, -1
            write(fname,'(i3,119x,2f7.2)')-ij+2, v(-ij+2), &
                    ((util(ij,1)/util(ij,0))**(1d0/egam)-1d0)*100d0
        enddo

        write(fname,'(i3,20f7.2)')0,CC(0),e(0),h(2,0),l(0),LL(0),KK(0),r(0),w(0), &
        ...
        do it = 1, TT
            ...
            write(fname,'(i3,20f7.2)')it,CC(it),e(it),h(2,it),l(it),LL(it),KK(it),r(it),w(it), &
    ...
    subroutine utility(it)
        ...
        do ijp = 1, JJ
            itp = year(it, 1, ijp)
            if(ijp /= JR)then
                util(1,it) = util(1,it)+beta**(ijp-1)*c(ijp,itp)**egam/egam
            else
                util(1,it) = util(1,it) + beta**(ijp-1)*(c(ijp,itp)**erho+nu*l(itp)**erho) &
                                                                **(egam/erho)/egam
            endif
        enddo
            ...
            do ij = 2, JJ
                ...
                do ijp = ij, JJ
                    ...
                    if(ijp /= JR)then
                        ...
                        util(ij,it) = util(ij,it) + beta**(ijp-ij)*(c(ijp,itp)**erho+nu*l(itp)**erho) &
                                                                       **(egam/erho)/egam
                    endif
                enddo
            enddo
