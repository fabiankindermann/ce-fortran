...
contains
    ...
    subroutine getedu(it)
        ...
        do ij = 2, JR-1
            itp = year(it, 1, ij)
            PRn = PRn*Rn(itp)
            pvw = pvw + w(itp)*(1d0-tauw(itp)-taup(itp))*(1d0-delh)**(ij-2)/PRn
        enddo
    ...
    subroutine get_path(ij,it)
        ...
        do ijp = ij+1, JJ
            ...
            a(ijp, ik) = wn(ijp-1,itm) + pen(ijp-1,itm)+ Rn(itm)*a(ijp-1,itm) - p(itm)*c(ijp-1,itm)
            if(ik == 2)a(ijp,ik) = a(ijp,ik)+v(-ijp+3)
            if(ik > 2 .and. ijp == 2) a(ijp,ik) = a(ijp,ik)+v(itm)
            if(ijp == 2) a(ijp,ik) = a(ijp,ik)-(1d0-taus(itm))*e(itm)*wn(ijp-1,itm)
        enddo
    ...
    subroutine output(it, fname)
        ...
        if(fname > 1)then
            write(fname,'(a)')' Age    cons       h        wn      pen    asset    Diff      '
            do ij = 1, JJ
                itp = year(it, 1, 2)
                if(ij < JJ)then
                    ...
                    diff = Rn(it)*a(ij,it)+wn(ij,it)+pen(ij,it)-p(it)*c(ij,it)
                endif
                if(ij == 1)then
                write(fname,'(i3,6f9.2)')ij,c(ij,it),h(ij,it),wn(ij,it)*(1d0-(1d0-taus(it))*e(it)),&
                            pen(ij,it),a(ij,it),diff
                else
                write(fname,'(i3,6f9.2)')ij,c(ij,it),h(ij,it),wn(ij,it),pen(ij,it),a(ij,it),diff
                endif
            enddo
            write(fname,'(a/)')'                                           '
        endif
