...
program ENGROWPIN_OLG
    ...
    real*8, parameter ::  delh    = 0.0d0
    real*8, parameter ::  xi      = 5.0d0
    real*8, parameter ::  upsi1   = 0.2d0
    real*8, parameter ::  upsi2   = 0.7d0
    ...
    real*8 :: by(0:TT), kappa(0:TT), np(0:TT), gy, tauk(0:TT)
    ...
    real*8 :: taus(0:TT), e(0:TT), HH(0:TT), ne(0:TT), XX(0:TT), pin(0:TT)
...
contains
    ...
    subroutine get_SteadyState
        ...
        do iter = 1, itermax
            ...
            if(abs(YY(0) - CC(0) - II(0) - GG(0) - XX(0))/YY(0) < tol) exit
        enddo

        if(iter < itermax)then
            write(*,'(a,i4,a,f16.10)')'Iteration: ',iter, ' Diff: ', &
                                    abs(YY(0)-CC(0)-II(0)-GG(0)-XX(0))/YY(0)
            write(*,*)
        else
    ...
    subroutine get_Transition()
        ...
        do iter = 1, itermax
            ...
            nmarket = 0
            do it = 1, TT
                if(abs(YY(it) - CC(it) - II(it) - GG(it) - TB(it) - XX(it))/YY(it) < tol*10d0) &
                        nmarket = nmarket + 1
            enddo
        ...
        write(*,'(a,i4,a,i4,a,f16.10)')'Iteration: ',iter,' Markets: ', nmarket,' Diff: ', &
                    maxval(abs(YY(:) - CC(:) - II(:) - GG(:) - TB(:) - XX(:))/YY(:))
    ...
    subroutine getedu(it)
        ...
        e(it) = (upsi1*xi*XX(it)**upsi2*pvw/(wn(it)*(1d0-taus(it))))**(1d0/(1d0-upsi1))
        if(e(it) > 0.99d0)e(it) = 0.99d0

        itp = year(it, 1, 2)
        ne(itp) = mu*(1d0 + xi*e(it)**upsi1*XX(it)**upsi2)-1d0
    ...
    subroutine quantities(it)
        ...
        ma(1,it) = 1d0
        do ij = 2, JJ
            ma(ij,it) =  ma(ij-1,itm)/(1d0+ne(it))/(1d0+np(it))
        enddo
        ...
        if(smopec .and. it > 0)then
            ...
            TB(it) = (1d0+np(itp))*(1d0+ne(itp))*BF(itp) - (1d0+r(it))*BF(it)
        else
            KK(it) = damp*(AA(it)-BB(it)) + (1d0-damp)*KK(it)
        endif
        II(it) = (1d0+np(itp))*(1d0+ne(itp))*KK(itp)-(1d0-delta)*KK(it)
    ...
    subroutine government(it)
        ...
        PEXP = taxrev(5)+(1d0+r(it))*BB(it)+GG(it)+XX(it)


        ! get budget balancing tax rate
        if(tax(it) == 1)then
            tauc(it) = (PEXP - (taxrev(2)+taxrev(3)+taxrev(4)+(1d0+np(itp))*(1d0+ne(itp))*BB(itp)))/CC(it)
        elseif(tax(it) == 2)then
            tauw(it) = (PEXP-(taxrev(1)+taxrev(4)+(1d0+np(itp))*(1d0+ne(itp))*BB(itp)))/(w(it)*LL(it)+r(it)*AA(it))
            taur(it) = tauw(it)
        elseif(tax(it) == 3)then
            tauw(it) = (PEXP-(taxrev(1)+taxrev(3)+ taxrev(4)+(1d0+np(itp))*(1d0+ne(itp))*BB(itp)))/(w(it)*LL(it))
        else
            taur(it) = (PEXP-(taxrev(1)+taxrev(2)+ taxrev(4)+(1d0+np(itp))*(1d0+ne(itp))*BB(itp)))/(r(it)*AA(it))
        endif
    ...
    ! subroutine for writing output to a file or the console (file = 0)
    subroutine output(it, file)
        ...
        write(file,'(a,i3/)')'Equilibrium: Year', it
        write(file,'(a)')' Goods Market  '
        write(file,'(a)')'      Y      C      G      I     TB    INF      DIFF'
        write(file,'(6f7.2,f10.4)')YY(it),CC(it),GG(it),II(it),TB(it),XX(it),diff
        write(file,'(6f7.2/)')YY(it)/YY(it),CC(it)/YY(it),GG(it)/YY(it),II(it)/YY(it),TB(it)/YY(it),XX(it)/YY(it)
        write(file,'(a)')' Capital Market  '
        write(file,'(a)')'      A      K     BB     BF       r'
        write(file,'(4f7.2,f8.2)')AA(it), KK(it), BB(it), BF(it), r(it)
        write(file,'(4f7.2/)')AA(it)/YY(it),KK(it)/YY(it),BB(it)/YY(it),BF(it)/YY(it)
        write(file,'(a)')' Labor Market  '
        write(file,'(a)')'     LL      e     ne      w   util'
        write(file,'(4f7.2,f7.3/)')LL(it),e(it),ne(it),w(it),util(1,it)
        write(file,'(a)')' GOVERMENT  '
        write(file,'(a)')'   tauc   tauw   taur   taup   tauk   taus    TXR     DD     rB  '
        write(file,'(9f7.2/)')tauc(it),tauw(it),taur(it),taup(it),tauk(it),taus(it),TXR(it)/YY(it),&
                            ((1+np(it1))*BB(it1)-BB(it))/YY(it),r(it)*BB(it)/YY(it)


        if(file > 1)then
            write(file,'(a)')' Age    cons       wn      pen    asset    Diff      '
            do ij = 1, JJ
                it1 = year(it, 1, 2)
                if(ij < JJ)then
                    diff = Rn(it)*a(ij,it)+winc(ij,it)+pen(ij,it)-a(ij+1,it1)-p(it)*c(ij,it)
!                     if(ij == 1) diff = diff - (1d0-taus(it))*e(it)*wn(it)
                    write(file,'(i3,5f9.2)')ij,c(ij,it),winc(ij,it),pen(ij,it),a(ij,it),diff
                else
                    diff = Rn(it)*a(ij,it)+pen(ij,it)-p(it)*c(ij,it)
                    write(file,'(i3,5f9.2)')ij,c(ij,it),0.00,pen(ij,it),a(ij,it),diff
                endif
            enddo
            write(file,'(a/)')'                                           '
        endif
    ...
    subroutine output_summary(file)
        ...
        ! headline
        write(file,'(a,a,a)')'      pin      C      e     ne      L      K      r', &
            '      w      Y      B     BA   tauc   tauw   taur', &
            '   taup   tauk   taus    HEV   y-d'
        diff = YY(0)-CC(0)-GG(0)-II(0)-XX(0)
        ! current generations
        do ij = JJ, 3, -1
            write(file,'(i3,119x,1f7.2)')-ij+2, &
                    ((util(ij,1)/util(ij,0))**(1d0/egam)-1d0)*100d0
        enddo
        write(file,'(i3,18f7.2)')0,pin(0),CC(0),e(0),ne(0),LL(0),KK(0),r(0),w(0), &
        ...
        do it = 1, TT
            ...
            write(file,'(i3,18f7.2)')it,pin(it),CC(it),e(it),ne(it),LL(it),KK(it),r(it),w(it), &
    ...
    ! subroutine for writing output summary to a file or the console (file = 0)
