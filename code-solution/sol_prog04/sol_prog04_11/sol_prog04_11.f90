...
program mortality_riskmanagement
    ...
    ! compute skewness and excess for portfolio
    do ii = 1, NN
        skew(ii) = sum((l(ii, :)-mu(ii))**3)/sig(ii, ii)**1.5d0/dble(KK)
        excess(ii) = sum((l(ii, :)-mu(ii))**4)/sig(ii, ii)**2/dble(KK) - 3d0
    enddo

    ! compute skewness and excess for portfolio
    skew_p = sum((l_p(:)-mu_p)**3)/sig_p**1.5d0/dble(KK)
    excess_p = sum((l_p(:)-mu_p)**4)/sig_p**2/dble(KK) - 3d0
    ...
    do ii = 2, NN
        write(*,'(9x,5f10.3)')sig(ii, :)
    enddo
    write(*,'(/a, 5f10.3)')'skew   = ', skew
    write(*,'(a, 5f10.3)')'excess = ', excess
    write(*,'(/2(a,f10.8,4x))')'mu_p   = ', mu_p, 'sig_p    = ', sig_p
    write(*,'(2(a,f10.8,4x))')'skew_p = ', skew_p, 'excess_p = ', excess_p
    write(*,'(2(a,f10.8,4x)/)')'VaR    = ', VaR, 'CVaR     = ', CVar

    !########################################################
    ! COMPUTE OPTIMAL PORTFOLIO
    !########################################################

    ! Compute restricted portfolio weights for different risk aversions
    do ig = 1, 3

        ! initialize communication variable for risk aversion
        gamma_com = gamma(ig)
    ...
    !########################################################
    ! PLOT THE MINUMU VARIANCE PORTFOLIOS
    !########################################################
    ...
    ! derive parameters
    a_mv = dot_product(matmul(IID, siginv), IID)
    b_mv = dot_product(matmul(IID, siginv), mu)
    c_mv = dot_product(matmul(mu, siginv), mu)

    ! create plot data for efficient portfolios
    do ii = 1, 1000
        x1(ii) = 1d0/1000d0*dble(ii+500)
    enddo
    y1 = (a_mv*x1**2 - 2d0*b_mv*x1 + c_mv)/(a_mv*c_mv - b_mv**2)

    call plot(y1, x1)
    call plot((/1d0/a_mv/), (/b_mv/a_mv/), marker=7, markersize=1.5d0, &
              noline=.true., legend='MV Portfolio')
    call plot((/sig_p_init/), (/mu_p_init/), marker=7, markersize=1.5d0, &
              noline=.true., legend='Stock 1')
    call execplot(xlabel='Portfolio Variance', ylabel='Portfolio Mean Return')

    !########################################################
    ! COMPUTE MINIMUM VARIANCE PORTFOLIO FOR GIVEN MUP
    !########################################################

    ! Portfolio weights for the minimum variance portfolio for a given mup
    omega = matmul(siginv, ((c_mv-b_mv*mu_p_init)/(a_mv*c_mv-b_mv**2))*IID &
                   + ((a_mv*mu_p_init-b_mv)/(a_mv*c_mv-b_mv**2))*mu)
    ...
    ! print results
    write(*,'(/a/)')'MINIMUM VARIANCE PORTFOLIO FOR GIVEN MUP - CHARACTERISTICS'
    write(*,'(a, 5f10.3)')'omega  = ', omega
    write(*,'(a, 5f10.3/)')'mu     = ', mu
    write(*,'(a, 5f10.3)')'sig    = ', sig(ii, :)
    do ii = 2, NN
    ...
    ! Portfolio weights for the minimum variance portfolio for a given mup
    omega = matmul(siginv, IID)/dot_product(matmul(IID, siginv), IID)
    ...
    ! print results
    write(*,'(/a/)')'GLOBAL MINIMUM VARIANCE PORTFOLIO - CHARACTERISTICS'
    write(*,'(a, 5f10.3)')'omega  = ', omega
    write(*,'(a, 5f10.3/)')'mu     = ', mu
    write(*,'(a, 5f10.3)')'sig    = ', sig(ii, :)
    do ii = 2, NN
