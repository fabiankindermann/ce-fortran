...
module globals
    ...
    real*8, parameter  :: gamma(3) = (/10d0, 15d0, 20d0/)     ! risk aversion
    ...
    real*8 :: mu(NN), sig(NN, NN), siginv(NN, NN), skew(NN), excess(NN)
    real*8 :: mu_p, sig_p, mu_p_init, sig_p_init, excess_p, skew_p, sum_premia
    real*8 :: VaR, CVaR, IID(NN), x_in(2*NN+1)
    real*8 :: a_mv, b_mv, c_mv, gamma_com, x1(1000), y1(1000)
    real*8 :: age(0:TT+1), PP_av(0:TT+1), PP_min(0:TT+1), PP_max(0:TT+1)
    integer :: ix, ig, ik, it, ik_max, ik_min, ii, im, id, ij, ii_VaR
    logical :: check
...
contains
    ...
    function focs(x_in)
        ...
        focs(1:NN) = - mu - gamma_com*matmul(sig, omega) - lamda + eta
