...
module globals
    ...
    integer :: ij_com, ip_com, is_com, it_com
    real*8 :: a_com, cons_com, lab_com, DIFF, INC_init

contains


    ! the first order condition (for calculation of Euler Equation Error)
    function foc_EER(x_in)

        implicit none
        real*8, intent(in) :: x_in
        real*8 :: foc_EER, a_plus, varphi, tomorrow, wage, available
        integer :: ial, iar
        ...
        available = (1d0+rn)*a_com + pen(ij_com)
        ...
        foc_EER = abs(1d0 - margu(cons_com, lab_com)/tomorrow**(-1d0/gamma))
