...
! PROGRAM SOLG_LR_EER
!
! ## Long-run equilibria in the stochastic OLG model (EER)
!
...
contains
    ...
    subroutine euler_equation_errors()

        implicit none
        integer, parameter :: n_err = 10000
        integer :: ij, ia, ip, ip_max, is, is_max, ial, iar
        real*8 :: cons, a_plus, varphi, EEE, EEE_a, err_temp

        ! calculate euler equation errors for each age ij
        do ij = 1, JJ-1

            EEE = 0d0
            EEE_a = 0d0

            ! check about how many is and ip to iterate
            if(ij >= JR)then
                ip_max = 1
                is_max = 1
            else
            ...
            do ip = 1, ip_max
                do is = 1, is_max
                    ...
                    do ia = 0, n_err
                        ...
                        ! linearly interpolate the savings function
                        ...
                        err_temp = foc_EER(a_plus)

                        ! check for greatest error
                        if(err_temp > EEE .and. a_plus > a(1))then
                            EEE = err_temp
                            EEE_a = a_com
                        endif
                    enddo
                enddo
            enddo
            write(*,'(a, i2, 2(a,es15.7))')'Euler equation error at age ',ij,' = ',EEE,' at a = ', EEE_a
        enddo
