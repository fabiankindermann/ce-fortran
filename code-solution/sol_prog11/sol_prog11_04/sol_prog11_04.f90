...
! PROGRAM SOLG_TR_EZ
!
! ## Implementation of Epstein-Zin preferences
!
...
program SOLG_TR_EZ
    ...
    tax(1:TT) = 2
    tauw(1:TT) = 0.19d0
    taur(1:TT) = 0.22d0    ! (use this for optimal progressivity only)
    tauc(1:TT) = tauc(0)   ! (use this for optimal lump sum tax: tauc = taur = tauw = 0.0)
    kappa(1:TT) = 0.42d0   ! kappa(0)
...
contains
        ...
        do ia = 0, NA
            ...
            do ip = 1, NP
                do is = 1, NS
                    ...
                    EV1(ij, ia, ip, is, it) = 0d0
                    do is_p = 1, NS
                        chelp = max(c(ij, ia, ip, is_p, it), 1d-10)
                        lhelp = max(l(ij, ia, ip, is_p, it), 1d-10)
                        EV(ij, ia, ip, is, it)  = EV(ij, ia, ip, is, it) + &
                                              pi(is, is_p)*max(VV(ij, ia, ip, is_p, it), 1d-10)**(1d0-chi)
                        EV1(ij, ia, ip, is, it)  = EV1(ij, ia, ip, is, it) + &
                                              pi(is, is_p)*max(VV(ij, ia, ip, is_p, it), 1d-10)** &
                                              (1d0/gamma-chi)*margu(chelp, lhelp, it)

                    enddo
                    EV(ij, ia, ip, is, it) = EV(ij, ia, ip, is, it)**(1d0/(1d0-chi))
                    RHS(ij, ia, ip, is, it) = max(((1d0+rn(it))*beta*EV(ij, ia, ip, is, it)**(chi-1d0/gamma)* &
                                                    EV1(ij, ia, ip, is, it)), 1d-10)**(-gamma)
    ...
    subroutine aggregation(it)
            ...
            do ia = 0, NA
                do ip = 1, NP
                    do is = 1, NS
                        ...
                        VV_coh(ij, it) = VV_coh(ij, it) &
                                  + max(VV(ij, ia, ip, is, it), 1d-10)**(1d0-chi)*phi(ij, ia, ip, is, it)
        ...
        VV_coh(:, it) = (VV_coh(:, it))**(1d0/(1d0-chi))/m_coh(:)
        FLC(:, it) = FLC(:, it)/m_coh(:)
    ...
    subroutine LSRA()
        ...
        real*8 :: VV_1, VV_0, dVV_da, v_tilde
        real*8 :: EVV_t, EVV_0, dEVV_da
                ...
                do ip = 1, NP
                    do is = 1, NS
                        ...
                        dVV_da = margu(c(ij, ia, ip, is, 1),l(ij, ia, ip, is, 1), 1)&
                                       *max(VV(ij, ia, ip, is, 1),1d-10)**(1d0/gamma)
        ...
        do it = TT, 1, -1
            ...
            do ip = 1, NP
                do is = 1, NS
                    dEVV_da = dEVV_da + VV_coh(1, it)**chi*max(VV(1, 0, ip, is, it),1d-10)**(1d0/gamma-chi) &
                              *margu(c(1, 0, ip, is, it), l(1, 0, ip, is, it), it)*phi(1, 0, ip, is, it)
                enddo
            enddo

            ! calculate present values
            if(it == TT)then
                PV_t     = EVV_t/dEVV_da    *(1d0+r(it))/(r(it)-n_p)
                PV_0     = EVV_0/dEVV_da    *(1d0+r(it))/(r(it)-n_p)
                PV_trans = v(1, 0, 1, 1, it)*(1d0+r(it))/(r(it)-n_p)
            else
                PV_t     = PV_t    *(1d0+n_p)/(1d0+r(it+1)) + EVV_t/dEVV_da
                PV_0     = PV_0    *(1d0+n_p)/(1d0+r(it+1)) + EVV_0/dEVV_da
    ...
    subroutine output_summary()
            ...
            do ia = 0, NA
                do ip = 1, NP
                    do is = 1, NS
                        ...
                        HEV_help = (VV(ij, ia, ip, is, 1)/max(VV(ij, ia, ip, is, 0), 1d-10)-1d0)*100d0
        ...
        do it = 1, TT
            HEV(it) = (VV_coh(1, it)/VV_coh(1, 0)-1d0)*100d0
        enddo

        ! headline
        write(22,'(/a,a)')'          A       K       L       H       r       w       C       I       Y', &
            '       B      BA    tauc    tauw    taur     Tr    taup     HEV      DIFF'
        ...
        do it = 0, TT
            write(22,'(i3,17f8.2,f10.5)')it,(/AA(it)/AA(0)-1d0, KK(it)/KK(0)-1d0, LL(it)/LL(0)-1d0, &
                ...
                tauw(it)-tauw(0), taur(it)-taur(0), Tr(it)/INC(0), taup(it)-taup(0)/)*100d0, &
