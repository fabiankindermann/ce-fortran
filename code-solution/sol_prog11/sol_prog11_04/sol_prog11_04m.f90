...
module globals
    ...
    real*8 :: YY(0:TT), CC(0:TT), II(0:TT), GG(0:TT), INC(0:TT)

    ! government variables
    real*8 :: tauc(0:TT), tauw(0:TT), taur(0:TT), taup(0:TT), Tr(0:TT), kappa(0:TT)
    real*8 :: gy, by, pen(JJ,0:TT), PP(0:TT), taxrev(5,0:TT)
    integer :: tax(0:TT)
    ...
    ! numerical variables
    real*8 :: RHS(JJ, 0:NA, NP, NS, 0:TT), EV(JJ, 0:NA, NP, NS, 0:TT), EV1(JJ, 0:NA, NP, NS, 0:TT)
...
contains
    ...
    function foc(x_in)
        ...
        available = (1d0+rn(it_com))*a(ia_com) + pen(ij_com, it_com) + v_ind
    ...
    function valuefunc(a_plus, cons, lab, ij, ip, is, it)
        ...
        valuefunc = 0d0
        if(ij < JJ)then
            valuefunc = max(varphi*EV(ij+1, ial, ip, is, itp) + (1d0-varphi)*EV(ij+1, iar, ip, is, itp), 1d-10)**egam
        endif

        ! add todays part and discount
        valuefunc = ((c_help**nu*(1d0-l_help)**(1d0-nu))**egam +  beta*valuefunc)**(1d0/egam)
