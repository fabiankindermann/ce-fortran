!##############################################################################
! PROGRAM SOLG_TR_SMOPEC
!
! ## The stochastic OLG model with progressive taxation (SMOPEC)
!
...
program SOLG_TR_SMOPEC
    ...
    tax(1:TT) = 5
    tauw(1:TT) = 0.13d0
    taur(1:TT) = 0.11d0  ! (use this for optimal progressivity only)
    tauc(1:TT) = tauc(0) ! (use this for optimal lump sum tax: tauc = taur = tauw = 0.0)
...
contains
    ...
    subroutine get_transition()
        ...
        do iter = 1, itermax
            ...
            itcheck = 0
            do it = 1, TT
                if(abs(DIFF(it)/YY(it))*100d0 < sig*2)itcheck = itcheck + 1
            enddo
            ...
            if(.not. lsra_on)then
                ...
                check = iter > 1 .and. itcheck == TT-2 .and. &
                ...
                check = iter > 1 .and. itcheck == TT-2 .and. lsra_comp/lsra_all > 0.99999d0 .and. &
                    abs(DIFF(itmax)/YY(itmax))*100d0 < sig*100d0
                ...
                do ip = 1, ip_max
                    do is = 1, is_max
                        ...
                        if(x_in < 0d0)then
                            ...
                            if(ij < JR)available = available + Tr(it)
                            if(ij < JR)then
        ...
        do ia = 0, NA
            do ip = 1, NP
                do is = 1, NS
                    ...
                    EV(ij, ia, ip, is, it) = (egam*EV(ij, ia, ip, is, it))**(1d0/egam)
                enddo
            enddo
        enddo
    ...
    subroutine aggregation(it)
        ...
        if(smopec .and. it > 0)then
            KK(it) = damp*(LL(it)*((r(it)+delta)/alpha/Omega)**(1d0/(alpha-1d0))) + (1d0-damp)*KK(it)
            BF(it) = AA(it) - KK(it) - BA(it) - BB(it)
            TB(it) = (1d0+n_p)*BF(itp) - (1d0+r(it))*BF(it)
        else
        ...
        DIFF(it) = YY(it)-CC(it)-II(it)-GG(it)-TB(it)
    ...
    subroutine government(it)
        ...
        if(tax(it) == 1)then
            tauc(it) = (expend + Tr(it)*workpop(it) - (tauw(it)*w(it)*LL(it) + taur(it)*r(it)*AA(it)))/CC(it)
            p(it)    = 1d0 + tauc(it)
        elseif(tax(it) == 2)then
            tauw(it) = (expend + Tr(it)*workpop(it) - tauc(it)*CC(it))/(w(it)*LL(it) + r(it)*AA(it))
            taur(it) = tauw(it)
        elseif(tax(it) == 3)then
            tauw(it) = (expend + Tr(it)*workpop(it) - (tauc(it)*CC(it) + taur(it)*r(it)*AA(it)))/(w(it)*LL(it))
        elseif(tax(it) == 4)then
            taur(it) = (expend + Tr(it)*workpop(it) - (tauc(it)*CC(it) + tauw(it)*w(it)*LL(it)))/(r(it)*AA(it))
        else
            Tr(it) = -(expend - (tauc(it)*CC(it) + tauw(it)*w(it)*LL(it) + taur(it)*r(it)*AA(it)))/workpop(it)
        endif
                ...
                do ip = 1, NP
                    do is = 1, NS
                        ...
                        v_tilde = max(v_tilde, -((1d0+rn(1))*a(ia) + pen(ij, 1) &
                            + wn(1)*eff(ij)*theta(ip)*eta(is)*0.99d0 + Tr(1) + v(ij, ia, ip, is, 1)))
    ...
    subroutine output(it)
        ...
        write(21,'(a)')'CAPITAL        K       A       B      BA      BF       r    p.a.'
        write(21,'(8x,7f8.2)')KK(it), AA(it), BB(it), BA(it),  BF(it), r(it) , ((1d0+r(it))**(1d0/5d0)-1d0)*100d0
        write(21,'(a,5f8.2/)')'(in %)  ',(/KK(it), AA(it), BB(it), BA(it), BF(it)/)/YY(it)*500d0

        write(21,'(a)')'LABOR          L      HH     INC       w'
        write(21,'(8x,4f8.2/)')LL(it), HH(it)*100d0, INC(it), w(it)

        write(21,'(a)')'GOODS          Y       C       I       G      TB    DIFF'
        write(21,'(8x,5f8.2,f8.3)')YY(it),CC(it),II(it),GG(it),TB(it),diff(it)
        write(21,'(a,5f8.2,f8.3/)')'(in %)  ',(/YY(it), CC(it), II(it), GG(it), TB(it), diff(it)/)/YY(it)*100d0

        write(21,'(a)')'GOV         TAUC    TAUW    TAUR   TRANS    TOTAL      G      rB'
        write(21,'(8x,7f8.2)')taxrev(1:5, it),GG(it),r(it)*BB(it)
        write(21,'(a,7f8.2)')'(in %)  ',(/taxrev(1:5, it), GG(it), r(it)*BB(it)*5d0/)/YY(it)*100d0
        write(21,'(a,4f8.2/)')'(rate)  ',(/tauc(it), tauw(it), taur(it)/)*100d0, Tr(it)/INC(0)*100d0
    ...
    subroutine output_summary()
        ...
        write(22,'(/a,a)')'          A       K       L       H       r       w       C       I       Y', &
            '       B      BA      BF      TB    tauc    tauw    taur     Tr    taup     HEV      DIFF'

        ! current generations
        do ij = -(JJ-2), -1
            write(22,'(i3,144x,f8.2)')ij,HEV(ij)
        enddo

        ! future generations
        do it = 0, TT
            write(22,'(i3,19f8.2,f10.5)')it,(/AA(it)/AA(0)-1d0, KK(it)/KK(0)-1d0, LL(it)/LL(0)-1d0, &
                HH(it)-HH(0), (1d0+r(it))**0.2d0-(1d0+r(0))**0.2d0, w(it)/w(0)-1d0, CC(it)/CC(0)-1d0, &
                II(it)/II(0)-1d0, YY(it)/YY(0)-1d0, BB(it)/YY(it)-BB(0)/YY(0), BA(it)*5/YY(it),BF(it)*5/YY(it),&
                TB(it)/YY(it),tauc(it)-tauc(0), tauw(it)-tauw(0), taur(it)-taur(0), Tr(it)/INC(0), &
                taup(it)-taup(0)/)*100d0, HEV(it), DIFF(it)/YY(it)*100d0
        enddo
