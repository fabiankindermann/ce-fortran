...
contains
    ...
    subroutine solve_household(ij_in, it_in)
        ...
        real*8 :: x_in, wage, available, v_ind, Xmax, fret
            ...
            do ia = 0, NA
                ...
                do ip = 1, ip_max
                    do is = 1, is_max
                        ...
                        Xmax = ((1d0+rn(it))*a(ia)+wn(it)*eff(ij)*theta(ip)*eta(is) &
                                + pen(ij,it)+v(ij, ia, ip, is, it))/p(it)
                        ...
                        call fminsearch(x_in, fret, 0d0, Xmax, valuefunc2)
                        ...
                        VV(ij, ia, ip, is, it) = valuefunc(x_in, cons_com, lab_com, ij, ip, is, it)!-fret
    ...
    subroutine LSRA()
        ...
        do it = TT, 1, -1
            ...
            do ip = 1, NP
                do is = 1, NS
                    dEVV_da = dEVV_da + margu(c(1, 0, ip, is, it), l(1, 0, ip, is, it), it)*phi(1, 0, ip, is, it)
                enddo
            enddo
    ...
    subroutine output_summary()
        ...
        ! headline
        write(22,'(/a,a)')'          A       K       L       H       r       w', &
            '       C       I       Y       B      BA    tauc    tauw    taur    taup     HEV      DIFF'
