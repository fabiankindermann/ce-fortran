...
!
! ## Solution with negative income tax
!
...
contains
    ...
    subroutine prices(it)
        ...
        rn(it) = r(it)*(1d0-taur(it)-tauy(it))
        wn(it) = w(it)*(1d0-tauw(it)-taup(it)-tauy(it))
        p(it) = 1d0 + tauc(it)
    ...
    subroutine solve_household(ij_in, it_in)
        ...
        do ia = 0, NA
            aplus(JJ, ia, :, :, it) = 0d0
            c(JJ, ia, :, :, it) = ((1d0+rn(it))*a(ia) + pen(JJ, it) + tauy(it)*try(it) + v(JJ, ia, :, :, it))/p(it)
            l(JJ, ia, :, :, it) = 0d0
                ...
                do ip = 1, ip_max
                    do is = 1, is_max
                        ...
                        if(x_in < 0d0)then
                            ...
                            available = (1d0+rn(it))*a(ia) + pen(ij, it) + tauy(it)*try(it) + v_ind
                            if(ij < JR)then
        ...
        do ia = 0, NA
            do ip = 1, NP
                do is = 1, NS
                    ...
                    EV(ij, ia, ip, is, it) = (egam*EV(ij, ia, ip, is, it))**egam
                enddo
            enddo
        enddo
    ...
    subroutine aggregation(it)
            ...
            do ia = 0, NA
                do ip = 1, NP
                    do is = 1, NS
                        ...
                        ! taxable income with negative income tax
                        ytax_coh(ij, it) = ytax_coh(ij, it) + (w(it)*eff(ij)*theta(ip)*eta(is)*l(ij, ia, ip, is, it) &
                                                +r(it)*a(ia)-try(it))*phi(ij, ia, ip, is, it)
        ...
        do ij = 1, JJ
            ...
            YTAX(it) = YTAX(it) + ytax_coh(ij, it)*m(ij, it)
    ...
    subroutine government(it)
        ...
        elseif(tax(it) == 2)then
            tauy(it) = (expend + Tr(it)*workpop(it) - tauc(it)*CC(it))/YTAX(it)
        ...
        taxrev(2, it) = tauy(it)*YTAX(it)
        taxrev(3, it) = tauw(it)*w(it)*LL(it)
        taxrev(4, it) = taur(it)*r(it)*AA(it)
        taxrev(5, it) = -Tr(it)*workpop(it)
        taxrev(6, it) = sum(taxrev(1:5, it))
    ...
    subroutine output(it)
        ...
        write(21,'(a)')'GOV         TAUC    TAUW    TAUR   TRANS    TAUY   TOTAL       G       B'
        write(21,'(8x,8f8.2)')taxrev(1:6, it),GG(it),BB(it)
        write(21,'(a,8f8.2)')'(in %)  ',(/taxrev(1:6, it), GG(it), BB(it)*5d0/)/YY(it)*100d0
        write(21,'(a,5f8.2/)')'(rate)  ',(/tauc(it), tauw(it), taur(it)/)*100d0, Tr(it)/INC(0)*100, tauy(it)*100d0
        ...
        do ij = 1, JJ
            write(21,'(i3,13f10.3,i10)')ij, c_coh(ij,it)/INC(0), l_coh(ij,it), (/w(it)*y_coh(ij,it), &
                    wn(it)*y_coh(ij,it)+rn(it)*a_coh(ij,it), tauy(it)*ytax_coh(ij, it), &
    ...
    subroutine output_summary()
        ...
        write(22,'(/a,a)')'          A       K       L       H       r       w       C       I       Y', &
            '       B      BA    tauc    tauw    taur     tauy    Tr    taup     HEV      DIFF'

        ! current generations
        do ij = -(JJ-2), -1
            write(22,'(i3,136x,f8.2)')ij,HEV(ij)
        enddo

        ! future generations
        do it = 0, TT
            write(22,'(i3,18f8.2,f10.5)')it,(/AA(it)/AA(0)-1d0, KK(it)/KK(0)-1d0, LL(it)/LL(0)-1d0, &
                ...
                tauw(it)-tauw(0), taur(it)-taur(0), tauy(it)- tauy(0), Tr(it)/INC(0), taup(it)-taup(0)/)*100d0, &
