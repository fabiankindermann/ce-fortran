...
!        omega(9) = 1d0/9d0
        ...
        do it = 0, TT
            ...
            itm = year2(it, -1)
            do ij = 2, JJ
                m(ij,it) = m(ij-1, itm)*psi(ij, it)/(1d0+n_p)
    ...
    subroutine prices(it)
        ...
        r(it) = Omega2*alpha*(KK(it)/LL(it))**(alpha-1d0)-delta
        w(it) = Omega2*(1d0-alpha)*(KK(it)/LL(it))**alpha
        rn(it) = r(it)*(1d0-taur(it))
        wn(it) = w(it)*(1d0-tauw(it)-taup(it))
        p(it) = 1d0 + tauc(it)
    ...
    subroutine solve_household(ij_in, it_in)
        ...
        beq(JJ, it) = damp*GAM(JJ, it)*BQ(it) + (1d0-damp)*beq(JJ, it)!

        do ia = 0, NA
            aplus(JJ, ia, :, :, it) = 0d0
            c(JJ, ia, :, :, it) = ((1d0+rn(it))*a(ia)+ beq(JJ, it) + pen(JJ, it) + v(JJ, ia, :, :, it))/p(it)!
            l(JJ, ia, :, :, it) = 0d0
                ...
                do ip = 1, ip_max
                    do is = 1, is_max
                        ...
                        if(x_in < 0d0)then
                            ...
                            available = (1d0+rn(it))*a(ia) + beq(ij, it) + pen(ij, it) + v_ind
                            if(ij < JR)then
        ...
        do ia = 0, NA
            ...
            do ip = 1, NP
                do is = 1, NS
                    ...
                    enddo
                    RHS(ij, ia, ip, is, it) = max((1d0+rn(it))*beta*psi(ij,it)*RHS(ij, ia, ip, is, it),1d-10)**(-gamma)
    ...
    subroutine aggregation(it)
            ...
            do ia = 0, NA
                do ip = 1, NP
                    do is = 1, NS
                        ...
                        beq_coh(ij, it) = beq_coh(ij, it) + a(ia)*(1d0 + rn(it))* &
                                                            (1d0 - psi(ij, it))*phi(ij, ia, ip, is, it)/psi(ij, it) !
                    enddo
                enddo
            enddo
        enddo
        ...
        YY(it) = Omega2 * KK(it)**alpha * LL(it)**(1d0-alpha)
    ...
    subroutine LSRA()
            ...
            do ia = 0, NA
                do ip = 1, NP
                    do is = 1, NS
                        ...
                        dVV_da = margu(c(ij, ia, ip, is, 1),l(ij, ia, ip, is, 1), 1)*(1+rn(1))

                        ! calculate change in transfers
                        v_tilde = (VV_0-VV_1)/dVV_da

                        ! restrict z_tilde to income maximum
                        v_tilde = max(v_tilde, -((1d0+rn(1))*a(ia) + beq(ij,1) + pen(ij, 1) &
        ...
        do it = TT, 1, -1
            ...
            do ip = 1, NP
                do is = 1, NS
                    dEVV_da = dEVV_da + margu(c(1, 0, ip, is, it), &
                                        l(1, 0, ip, is, it), it)*phi(1, 0, ip, is, it)*(1d0+rn(it))
                enddo
            enddo
    ...
    subroutine output_summary()
        ...
        ! headline
        write(22,'(/a,a,a)')'          A       K       L       H       r       ', &
                            'w       C       I       Y       B      BA    tauc    tauw    taur', &
                            '    taup     HEV      DIFF       v_coh'
        ...
        if(lsra_on)write(22, '(/a,f12.6)')'EFFICIENCY GAIN: ', (Vstar**(1d0/egam)-1d0)*100d0
