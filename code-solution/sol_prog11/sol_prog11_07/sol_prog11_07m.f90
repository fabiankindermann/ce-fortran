...
module globals
    ...
    real*8, parameter :: Omega2 = 1.60d0

    ! size of the asset grid
    real*8, parameter :: a_l    = 0.0d0
    real*8, parameter :: a_u    = 50d0
    real*8, parameter :: a_grow = 0.05d0

    ! demographic parameters
    real*8, parameter :: n_p   = (1d0+0.02d0)**5-1d0
    ...
    real*8 :: YY(0:TT), CC(0:TT), II(0:TT), GG(0:TT), INC(0:TT), BQ(0:TT)

    ! government variables
    real*8 :: tauc(0:TT), tauw(0:TT), taur(0:TT), taup(0:TT), kappa(0:TT)
    real*8 :: gy, by, pen(JJ,0:TT), PP(0:TT), taxrev(4,0:TT)
    integer :: tax(0:TT)
    ...
    real*8 :: v_coh(JJ, 0:TT), VV_coh(JJ, 0:TT) = 0d0
    real*8 :: GAM(JJ, 0:TT), beq(JJ, 0:TT), omega(JJ), beq_coh(JJ,0:TT), psi(JJ+1, 0:TT)!
...
contains
    ...
    function foc(x_in)
        ...
        available = (1d0+rn(it_com))*a(ia_com) + beq(ij_com, it_com) + pen(ij_com, it_com) + v_ind
    ...
    function valuefunc(a_plus, cons, lab, ij, ip, is, it)
        ...
        valuefunc = (c_help**nu*(1d0-l_help)**(1d0-nu))**egam/egam + beta*psi(ij+1, itp)*valuefunc
