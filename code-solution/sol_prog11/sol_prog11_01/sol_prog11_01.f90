...
! PROGRAM SOLG_TR_AS
!
! ## The stochastic OLG model with progressive taxation and absence of shocks
!
...
program SOLG_TR_AS
    ...
    tax(1:TT) = 5
    tauw(1:TT) = 0.0d0
    taur(1:TT) = 0.0d0 ! taur(0)    ! (use this for optimal progressivity only)
        ...
        do iter = 1, itermax
            ...
            if(.not. lsra_on)then
                ...
                write(*,'(i4,3f12.5)')iter, lsra_comp/lsra_all*100d0, &
                    (Lstar**(1d0/egam)-1d0)*100d0,DIFF(itmax)/YY(itmax)*100d0
            endif
