...
module globals
    ...
    integer :: lambda
    real*8 :: Ybarn(2), q(2,2), p(2,2), w(2), wn(2), rn(2), T(2,2), wh(2,2)
    real*8 :: Xd(2,2), Y(2,2), ky(2,2), ly(2,2), K(2,2), L(2,2), U(2), ell(2)
...
contains
    ...
    function markets(x)
        ...
        q(:,1) = (w(1)*ly(:,1)+r*ky(:,1))*(1d0+lambda*tauc(:,1))
        call lu_solve(ID-transpose(a), q(:,1))
        q(1,2) = (a(2,1)*q(2,1)+(w(2)*ly(1,2)+r*ky(1,2))*(1d0+lambda*tauc(1,2)))/(1d0-a(1,1))
        q(2,2) = q(2,1)

        ! 3. consumer prices and demands
        do i = 1, 2
            p(:,i) = q(:,i)*(1d0+(1d0-lambda)*tauc(:,i))
            wn(i) = w(i)*(1d0-tauw(i))
            rn(i) = r*(1d0-taur(i))
        ...
        markets(3) = q(1,1)*G-sum(T(:,1))-&
            tauw(1)*w(1)*(Tbar(1)-ell(1))-taur(1)*r*Kbar(1)
        markets(4) = q(1,2)*G-sum(T(:,2))-&
