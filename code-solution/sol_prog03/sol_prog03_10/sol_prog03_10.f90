!##############################################################################
! PROGRAM taxreformint
!
! ## The static GE model with international trade and tax reform
!
...
program taxreformint
    ...
    ! initial guess
    x(:) = (/1d0, 0.5d0, 0.01d0, 0.2d0/)

    ! set DP (lambda = 0) or OP (lambda = 1)
    lambda = 1
    ...
    do i = 1, 2
        ...
        write(*,'(/a)')'GOVERNMENT :'
        write(*,'(6(a,f6.2,2x))')' tc1=',T(1,i), &
            ' tc2=',T(2,i),' tw =',tauw(i)*w(i)*(Tbar(i)-ell(i)), &
