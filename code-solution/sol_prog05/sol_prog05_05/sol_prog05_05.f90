...
! PROGRAM CARAutility
!
! ## The life cycle model with CARA utility
!
...
program CARAutility
    ...
    real*8 :: income(n_w*n_R), share(n_w*n_R)
    integer :: ind(n_w*n_R)
    ...
    if(sig_w > 0 .or. sig_R > 0)then
        income(:) = wR(:, 1) + (Rf+omega(1, 1)*(wR(:, 2)-Rf))*a(2, 1)
        share(:) = omega(2, :)
        call sort(income, ind)
        share(:) = share(ind)
        call plot(income(:), share)
        call execplot(xlabel='Income', ylabel='Equity share')
    endif
