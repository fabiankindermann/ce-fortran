...
module globals
    ...
    real*8 :: R(n_R), weight_R(n_R), u(3,n_w*n_R,n_R)
...
contains
    ...
    function utility(x)
        ...
        do iw = 1, n_w
            do ir2 = 1, n_R
                do ir3 = 1, n_R
                    ...
                    u(2, iwR, :) = 1d0 - exp(-c(2, iwR, 1)/gamma)
                    u(3, iwR, ir3) = 1d0 - exp(-c(3, iwR, ir3)/gamma)
                    utility = utility + prob*(u(2, iwR, 1) + beta*u(3, iwR, ir3))
                enddo
                iwR = iwR + 1
            enddo
        enddo
        ...
        ! utility function
        u(1, :, :) = 1d0-exp(-c(1, 1, 1)/gamma)
        utility  = -(u(1, 1, 1) + beta*utility)
