...
! PROGRAM Entrycost
!
! ## The life cycle model with portfolio choice and entry cost
!
...
program Entrycost
    ...
    implicit none
    real*8 :: x1(2*(1+n_w*n_R)), x2(1+n_w*n_R)
    real*8 :: fret1, low1(2*(1+n_w*n_R)), up1(2*(1+n_w*n_R))
    real*8 :: fret2, low2(1+n_w*n_R), up2(1+n_w*n_R)
    ...
    ! lower and upper border and initial guess for stock investment
    low1 = 0d0
    up1(1) = mu_w
    up1(2) = 1d0
    ic = 3
    ...
    call fminsearch(x1, fret1, low1, up1, utility_st)

    ! lower and upper border and initial guess for bond investment
    low2 = 0d0
    up2(1) = mu_w
    ic = 2
    iwR = 1
    ...
    ! minimization routine
    call fminsearch(x2, fret2, low2, up2, utility_b)
    omega = 0d0

    ! check whether market entry is optimal
    if(-fret1 > -fret2)then
        fret1 = utility_st(x1)
    endif
