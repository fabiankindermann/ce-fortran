...
module globals
    ...
    ! transfer payments (old-age), borrowing constraint, efficiency and survival probabilities
    ...
    real*8 :: a(0:NA), aplus(JJ, 0:NA, NP, NS, 0:1), c(JJ, 0:NA, NP, NS, 0:1)
    real*8 :: phi(JJ, 0:NA, NP, NS, 0:1), V(JJ, 0:NA, NP, NS, 0:1) = 0d0

    ! cohort aggregate variables
    real*8 :: c_coh(JJ, 0:1), y_coh(JJ, 0:1), a_coh(JJ, 0:1), v_coh(JJ, 0:1)
    real*8 :: cv_c(JJ, 0:1), cv_y(JJ, 0:1)

    ! numerical variables
    real*8 :: RHS(JJ, 0:NA, NP, NS), EV(JJ, 0:NA, NP, NS, 0:1)
    integer :: ij_com, ia_com, ip_com, is_com
    real*8 :: cons_com, hdelta_com
...
contains
    ...
    function foc(x_in)
        ...
        wage = eff(ij_com)*theta(ip_com)*eta(is_com)
        ...
        foc = cons_com - hdelta_com**(-gamma)*tomorrow
    ...
    function valuefunc(a_plus, cons, ij, ip, is, ih)

        implicit none
        integer, intent(in) :: ij, ip, is, ih
        ...
        valuefunc = 0d0
        if(ij < JJ)then
            valuefunc = max(varphi*EV(ij+1, ial, ip, is, ih) + &
                            (1d0-varphi)*EV(ij+1, iar, ip, is, ih), 1d-10)**egam/egam
        endif
