...
contains
    ...
    subroutine interpolate(ij)
        ...
        integer :: ia, iw, isr
        real*8 :: X_p, earnings, c_p, varphi, dist, EV, R_port
        integer :: ixl, ixr
        ...
        do ia = 0, NA
            ...
            if(ij >= JR-1)then
                ...
                do isr = 1, NSR
                    ...
                    EV = varphi*V(ij+1, ixl) + (1d0-varphi)*V(ij+1, ixr)

                    ! get RHS of foc and Q
                    RHS(ij, ia) = RHS(ij, ia) + dist*R_port*EV**(1d0/gamma-chi)*c_p**(-1d0/gamma)
                    Q(ij, ia)   = Q(ij, ia) + dist*EV**(1d0-chi)
                enddo
            ...
            ! agent is working
                ...
                do iw = 1, NW
                    do isr = 1, NSR
                        ...
                        ! get RHS of foc and Q
                        RHS(ij, ia) = RHS(ij, ia) + dist*R_port*EV**(1d0/gamma-chi)* &
                                      eps(isr)**(-chi)*c_p**(-1d0/gamma)
                        Q(ij, ia)   = Q(ij, ia) + dist*EV**(1d0-chi)
                    enddo
                enddo
            endif

            Q(ij, ia)   = Q(ij, ia)**(1d0/(1d0-chi))
            RHS(ij, ia) = (beta*psi(ij+1))**(-gamma)*Q(ij, ia)**(1d0-chi*gamma)*RHS(ij, ia)**(-gamma)
        enddo
