...
contains
    ...
    function foc_port(p)
        ...
        implicit none
        real*8, intent(in) :: p
        real*8 :: foc_port, omega_p, R_port, X_p, earnings, c_p, varphi, dist, V_p, EV
        ...
        if(ij_com+1 >= JR)then
            do isr = 1, NSR
                ...
                ! get FOC
                foc_port = foc_port + dist*(mu_r + vtheta(isr))*a(ia_com)* &
                           V_p**(1d0/gamma-chi)*c_p**(-1d0/gamma)
                EV = EV + dist*V_p**(1d0-chi)
            enddo
        else
            do iw = 1, NW
                do isr = 1, NSR
                    ...
                    ! get FOC
                    foc_port = foc_port + dist*(mu_r + vtheta(isr))*a(ia_com)* &
                               V_p**(1d0/gamma - chi)*eps(isr)**(-chi)*c_p**(-1d0/gamma)
                    EV = EV + dist*(eps(isr)*V_p)**(1d0-chi)
                enddo
            enddo
        endif

        EV = EV**(1d0/(1d0-chi))
        foc_port = foc_port*EV**chi
    ...
    function valuefunc(a_plus, cons, ij)
        ...
        if(ij < JJ)then
            ...
            valuefunc = max(varphi*Q(ij, ial) + (1d0-varphi)*Q(ij, iar), 1d-10)
            valuefunc = (c_help**egam + beta*psi(ij+1)*valuefunc**egam)**(1d0/egam)
        else
            valuefunc = c_help
        endif
