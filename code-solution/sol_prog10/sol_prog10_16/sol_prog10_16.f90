!##############################################################################
! PROGRAM PortfolioChoiceVFI
!
! ## Portfolio choice in the life cycle model with value function iteration
!
...
contains
    ...
    subroutine solve_household()
        ...
        do ij = JJ-1, 1, -1
            ...
            ! evaluate intermediate value function Q at zero savings
            ij_com = ij
            ia_com = 0
            Q(ij, 0) = (-egam*value_port(omega_plus(ij, 0)))**(1d0/egam)
    ...
    subroutine solve_portfolio(ij, ia)
        ...
        x_in = omega_plus(ij+1, ia)

        ! find optimal portfolio share
        call fminsearch(x_in, fret, 0d0, 1d0, value_port)

        ! copy decision
        omega_plus(ij, ia) = x_in
        Q(ij, ia) = (-egam*fret)**(1d0/egam)
    ...
    subroutine solve_consumption(ij, ix)
        ...
        call fminsearch(x_in, fret, 0d0, X(ix), value_cons)

        ! copy decisions
        a_plus(ij, ix) = x_in
        c(ij, ix) = cons_com
        V(ij, ix) = (-egam*fret)**(1d0/egam)
    ...
    subroutine output()
        ...
        call plot(dble(ages), y_coh+pen, legend='Earnings (Mean)')
