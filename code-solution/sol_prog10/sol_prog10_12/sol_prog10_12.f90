!##############################################################################
! PROGRAM FemaleParticipationChildren
!
...
contains
    ...
    subroutine solve_household()
        ...
        real*8 :: x_in, wage_m, wage_f, fret, aplus_max
        ...
        do ia = 0, NA
            ...
            l(JJ, ia, :, :, :, :) = 0d0
            V(JJ, ia, :, :, :, :) = (c(JJ, ia, 0, 1, 1, 1)/sqrt(2d0+dble(nchild(JJ))))**egam/egam
        enddo

        ! interpolate individual EV
        call interpolate(JJ)
                        ...
                        do ism = 1, is_max
                            do isf = 1, is_max
                                ...
                                do il = 0, il_max
                                    ...
                                    aplus_max = (1d0+r)*a(ia) + w_m*eff(ij)*theta(ip)*eta(ism) + &
                                        w_f*dble(il)*(exp(h(ih))*theta(ip)*eta(isf) - pchild(ij)) + pen(ij)

                                    ! maximize value function
                                    call fminsearch(x_in, fret, a_l, min(aplus_max, a_u), valuefunc)
