!##############################################################################
! PROGRAM BaselineWhiteNoise
!
! ## The baseline life cycle model with earnings risk (including white noise)
!
...
contains
    ...
    subroutine initialize
        ...
        ! discretize the white noise process
        call normal_discrete(white, dist_white, 0d0, sigma_white)
        white = exp(white)
        ...
        do ij = 1, JJ
            do ip = 1, NP
                ...
                abor_temp = 0d0
                do ijj = JR, ij, -1
                    abor_temp = abor_temp/(1d0+r) + eff(ijj)*theta(ip)*eta(1)*white(1) + pen(ijj)
                enddo
    ...
    subroutine solve_household()
        ...
        implicit none
        integer :: ij, ia, ip, ip_max, is, is_max, iw, iw_max
        ...
        do ia = 0, NA
            ...
            V(JJ, ia, :, :, :) = valuefunc(0d0, c(JJ, ia, 1, 1, 1), JJ, 1, 1)
        enddo
        ...
        do ij = JJ-1, 1, -1
            ...
            do ia = 0, NA
                ...
                do ip = 1, ip_max
                    do is = 1, is_max
                        do iw = 1, iw_max
                            ...
                            if(check)write(*,'(a, 5i4)')'ERROR IN ROOTFINDING : ', ij, ia, ip, is, iw

                            ! check for borrowing constraint
                            if(x_in < 0d0)then
                                x_in = 0d0
                                wage = w*eff(ij)*theta(ip)*eta(is)*white(iw)
                                available = (1d0+r)*(a(ia) + a_bor(ij, ip)) + wage + pen(ij)
                                cons_com = available - a_bor(ij+1, ip)
                            endif

                            ! copy decisions
                            aplus(ij, ia, ip, is, iw) = x_in
                            c(ij, ia, ip, is, iw) = cons_com
                            V(ij, ia, ip, is, iw) = valuefunc(x_in, cons_com, ij, ip, is)
                        enddo
                    enddo
                enddo
                ...
                if(ij >= JR)then
                    aplus(ij, ia, :, :, :) = aplus(ij, ia, 1, 1, 1)
                    c(ij, ia, :, :, :) = c(ij, ia, 1, 1, 1)
                    V(ij, ia, :, :, :) = V(ij, ia, 1, 1, 1)
                endif
            enddo
        ...
        do ia = 0, NA
            do ip = 1, NP
                do is = 1, NS
                    ...
                    do is_p = 1, NS
                        do iw = 1, NW
                            chelp = max(c(ij, ia, ip, is_p, iw), 1d-10)
                            RHS(ij, ia, ip, is) = RHS(ij, ia, ip, is) + &
                                                  pi(is, is_p)*dist_white(iw)*margu(chelp)
                            EV(ij, ia, ip, is) = EV(ij, ia, ip, is) + &
                                                 pi(is, is_p)*dist_white(iw)*V(ij, ia, ip, is_p, iw)
                        enddo
                    enddo
    ...
    subroutine get_distribution()
        ...
        implicit none
        integer :: ij, ia, ip, is, is_p, iw, iw_p, ial, iar
        real*8 :: varphi
        ...
        do ip = 1, NP
            ...
            do iw = 1, NW
                phi(1, ial, ip, is_initial, iw) = varphi*dist_theta(ip)*dist_white(iw)
                phi(1, iar, ip, is_initial, iw) = (1d0-varphi)*dist_theta(ip)*dist_white(iw)
            enddo
        enddo
                ...
                do ip = 1, NP
                    do is = 1, NS
                        do iw = 1, NW
                            ...
                            call linint_Grow(aplus(ij-1, ia, ip, is, iw), a_l, a_u, a_grow, NA, ial, iar, varphi)
                            ...
                            do is_p = 1, NS
                                do iw_p = 1, NW
                                    phi(ij, ial, ip, is_p, iw_p) = phi(ij, ial, ip, is_p, iw_p) + &
                                        pi(is, is_p)*dist_white(iw_p)*varphi*phi(ij-1, ia, ip, is, iw)
                                    phi(ij, iar, ip, is_p, iw_p) = phi(ij, iar, ip, is_p, iw_p) + &
                                        pi(is, is_p)*dist_white(iw_p)*(1d0-varphi)*phi(ij-1, ia, ip, is, iw)
                                enddo
                            enddo
                        enddo
                    enddo
        ...
        do ij = 1, JJ
            do ia = 0, NA
                do ip = 1, NP
                    do is = 1, NS
                        do iw = 1, NW
                            c_coh(ij) = c_coh(ij) + c(ij, ia, ip, is, iw)*phi(ij, ia, ip, is, iw)
                            y_coh(ij) = y_coh(ij) + eff(ij)*theta(ip)*eta(is)*white(iw)*phi(ij, ia, ip, is, iw)
                            a_coh(ij) = a_coh(ij) + (a(ia)+a_bor(ij, ip))*phi(ij, ia, ip, is, iw)
                            v_coh(ij) = v_coh(ij) + V(ij, ia, ip, is, iw)*phi(ij, ia, ip, is, iw)
                        enddo
                    enddo
                enddo
            enddo
        ...
        do ij = 1, JJ
            do ia = 0, NA
                do ip = 1, NP
                    do is = 1, NS
                        do iw = 1, NW
                            cv_c(ij) = cv_c(ij) + c(ij, ia, ip, is, iw)**2*phi(ij, ia, ip, is, iw)
                            cv_y(ij) = cv_y(ij) + (eff(ij)*theta(ip)*eta(is)*white(iw))**2*phi(ij, ia, ip, is, iw)
                        enddo
                    enddo
                enddo
            enddo
    ...
    subroutine output()
        ...
        implicit none
        integer :: ij, ia, ip, is, iw, iamax(JJ), ages(JJ)
        real*8 :: frac_bor(JJ)
        ...
        do ij = 1, JJ - 1
            do ia = 0, NA
                do ip = 1, NP
                    do is = 1, NS
                        do iw = 1, NW
                            if(aplus(ij, ia, ip, is, iw) < 1d-6)then
                                frac_bor(ij) = frac_bor(ij) + phi(ij, ia, ip, is, iw)
                            endif
                        enddo
                    enddo
                enddo
    ...
    subroutine check_grid(iamax)
        ...
        implicit none
        integer :: iamax(JJ), ij, ia, ip, is, iw
        ...
        do ij = 1, JJ
            ...
            do ia = 0, NA
                do ip = 1, NP
                    do is = 1, NS
                        do iw = 1, NW
                            if(phi(ij, ia, ip, is, iw) > 1d-8)iamax(ij) = ia
                        enddo
                    enddo
                enddo
            enddo
