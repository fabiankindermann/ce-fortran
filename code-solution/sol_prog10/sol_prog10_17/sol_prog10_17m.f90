...
module globals
    ...
    ! number of different fixed cost states
    ...
    real*8 :: c_coh(JJ), y_coh(JJ), a_coh(JJ), o_coh(JJ), e_coh(JJ)
    real*8 :: cv_c(JJ), cv_y(JJ), cv_a(JJ), cv_o(JJ)

    ! individual variables
    real*8 :: X(0:NX), a(0:NA)
    real*8 :: c(JJ, 0:NX, 0:NP), a_plus(JJ, 0:NX, 0:NP), V(JJ, 0:NX, 0:NP) = 0d0
    real*8 :: omega_plus(JJ, 0:NA, 0:NP), Q(JJ, 0:NA, 0:NP)
    real*8 :: FF, e_plus(JJ, 0:NX, 0:NP) = 0d0
    real*8 :: phi_X(JJ, 0:NX, 0:NP), phi_a(JJ, 0:NA, 0:NP)
    real*8 :: eta(JJ, 0:NE), phi_e(JJ, 0:NE)

    ! numerical variables
    real*8 :: RHS(JJ, 0:NA, 0:NP)
    integer :: ij_com, ix_com, ia_com, ip_com, ip_p_com
    real*8 :: cons_com
...
contains
    ...
    function foc_cons(x_in)
        ...
        cons_com = X(ix_com) - a_plus - dble(ip_p_com-ip_com)*FF

        ! calculate linear interpolation for future part of first order condition
        call linint_Grow(a_plus, a_l, a_u, a_grow, NA, ial, iar, varphi)

        tomorrow = varphi*RHS(ij_com, ial, ip_p_com) + (1d0-varphi)*RHS(ij_com, iar, ip_p_com)
    ...
    function foc_port(p)
        ...
        if(ij_com+1 >= JR)then
            do isr = 1, NSR
                ...
                c_p = varphi      *c(ij_com+1, ixl, ip_com) + &
                      (1d0-varphi)*c(ij_com+1, ixr, ip_com)
                c_p = max(c_p, 1d-10)
    ...
    function valuefunc(a_plus, cons, ij, ip)
        ...
        valuefunc = 0d0
        if(ij < JJ)then
            valuefunc = max(varphi*Q(ij, ial, ip) + (1d0-varphi)*Q(ij, iar, ip), 1d-10)**egam/egam
        endif
