...
module globals
    ...
    real*8, parameter :: sigma_zeta = 0.2d0
    real*8, parameter :: c_floor    = 0.05d0
    ...
    ! the health shock
    real*8 :: dist_m(0:NM), pi_m(JJ, NP, 0:NM, 0:NM)

    ! the health cost shock
    real*8 :: dist_zeta(NH), zeta(NH)
    real*8 :: hc(JJ, NH), varrho(NP), varrho_m(NP, 0:NM)
    ...
    real*8 :: a(0:NA), aplus(JJ, 0:NA, NP, 0:NM, NS, NH), c(JJ, 0:NA, NP, 0:NM, NS, NH)
    real*8 :: phi(JJ, 0:NA, NP, 0:NM, NS, NH), V(JJ, 0:NA, NP, 0:NM, NS, NH) = 0d0

    ! cohort aggregate variables
    real*8 :: c_coh(JJ, NP+1, 0:NM+1), y_coh(JJ, NP+1, 0:NM+1), a_coh(JJ, NP+1, 0:NM+1)
    real*8 :: v_coh(JJ, NP+1, 0:NM+1), cv_c(JJ), cv_y(JJ), frac_phi(JJ, NP, 0:NM+1)

    ! numerical variables
    real*8 :: RHS(JJ, 0:NA, NP, 0:NM, NS), EV(JJ, 0:NA, NP, 0:NM, NS)
    integer :: ij_com, ia_com, ip_com, im_com, is_com, ih_com
    real*8 :: cons_com
...
contains
    ...
    function foc(x_in)
        ...
        real*8 :: foc, a_plus, varphi, tomorrow, wage, btran, available
        integer :: ial, iar
        ...
        btran = max(c_floor + hc(ij_com, ih_com) - (1d0+r)*(a(ia_com) + &
                        a_bor(ij_com, ip_com)) - wage - pen(ij_com), 0d0)
        available = (1d0+r)*(a(ia_com) + a_bor(ij_com, ip_com)) + wage + pen(ij_com) - hc(ij_com, ih_com) + btran
