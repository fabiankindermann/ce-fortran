...
module globals
    ...
    ! transfer payments (old-age), survival probabilities, productivity, bequest
    real*8 :: pen(JJ), psi(JJ+1), eff(JJ), nu(JJ+1)

    ! individual variables
    real*8 :: a(0:NA), aplus(JJ+1, 0:NA, NP, NS), c(JJ, 0:NA, NP, NS)
    ...
    ! numerical variables
    real*8 :: RHS(JJ+1, 0:NA, NP, NS), EV(JJ+1, 0:NA, NP, NS)
...
contains
    ...
    function valuefunc(a_plus, cons, ij, ip, is)
        ...
        valuefunc = c_help**egam/egam + beta*(psi(ij+1)*valuefunc + &
            (1d0-psi(ij+1))*nu(ij)*(1d0 + (1d0+r)*a_plus/chi)**egam/egam)
