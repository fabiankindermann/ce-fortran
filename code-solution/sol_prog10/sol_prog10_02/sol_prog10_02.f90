!############################################################################
! PROGRAM BaselineBequest
!
! ## The baseline life cycle model with bequest motive
!
...
contains
    ...
    subroutine solve_household()
        ...
        implicit none
        integer :: ij, ia, ip, is, ip_max, is_max, JX
                ...
                do ip = 1, ip_max
                    do is = 1, is_max
                        ...
                        if(x_in < 0d0)then
                            ...
                            available = (1d0+r)*a(ia) + wage + pen(ij)
                            cons_com = available
                        endif
        ...
        do ia = 0, NA
            do ip = 1, NP
                do is = 1, NS
                    ...
                    if(ij > JJ)then
                        RHS(ij, ia, ip, is) = beta*(nu(ij)*(1d0+r)/chi)**(-gamma)*(1d0+(1d0+r)*a(ia)/chi)
                        EV(ij, ia, ip, is) = beta*nu(ij)**(1d0/egam)*(1d0+(1d0+r)*a(ia)/chi)
                    else
                        RHS(ij, ia, ip, is) = 0d0
                        EV(ij, ia, ip, is) = 0d0
                        do is_p = 1, NS
                        ...
                        enddo
                        RHS(ij, ia, ip, is) = (beta*(psi(ij)*(1d0+r)*RHS(ij, ia, ip, is) + &
                            (1d0-psi(ij))*nu(ij)*(1d0+r)/chi*(1d0+(1d0+r)*a(ia)/chi)**(-1d0/gamma)))**(-gamma)
                        EV(ij, ia, ip, is) = (egam*beta*(psi(ij)*EV(ij, ia, ip, is) + &
                            (1d0-psi(ij))*nu(ij)*(1d0+(1d0+r)*a(ia)/chi)**egam/egam))**(1d0/egam)
                    endif
                enddo
            enddo
        enddo
    ...
    subroutine get_distribution()
        ...
        do ip = 1, NP
            phi(1, 0, ip, is_initial) = varphi*dist_theta(ip)
            phi(1, 0, ip, is_initial) = (1d0-varphi)*dist_theta(ip)
        enddo
    ...
    subroutine output()
        ...
        do ij = 1, JJ
            write(21,'(i3,8f10.3,i10)')ages(ij), c_coh(ij), y_coh(ij), y_coh(ij)+r*a_coh(ij), pen(ij), a_coh(ij), &
