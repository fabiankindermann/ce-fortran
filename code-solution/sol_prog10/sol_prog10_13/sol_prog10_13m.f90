...
module globals
    ...
    real*8 :: a(0:NA), h(0:NH), aplus(JJ, 0:NA, 0:NH, NP, 0:NS, 0:NS)
    real*8 :: c(JJ, 0:NA, 0:NH, NP, 0:NS, 0:NS), l(JJ, 0:NA, 0:NH, NP, 0:NS, 0:NS)
    real*8 :: phi(JJ, 0:NA, 0:NH, NP, 0:NS, 0:NS), V(JJ, 0:NA, 0:NH, NP, 0:NS, 0:NS) = 0d0
    ...
    ! numerical variables
    real*8 :: RHS(0:NA, 0:NH, NP, 0:NS, 0:NS, 0:1), EV(0:NA, 0:NH, NP, 0:NS, 0:NS, 0:1)
...
contains
    ...
    function foc(x_in)
        ...
        if(ism_com > 0 .and. isf_com > 0)then
            wage_f = w_f*dble(il_com)*(exp(h(ih_com))*theta(ip_com)* &
                      eta(isf_com) - pchild(ij_com))
            cons_com = (1d0+r)*a(ia_com) + pen(ij_com) + wage_f + wage_m - a_plus
        elseif(isf_com == 0)then
            cons_com = (1d0+r)*a(ia_com) + 0.5d0*pen(ij_com) + wage_m - a_plus
        elseif(ism_com == 0)then
            wage_f = w_f*dble(il_com)*exp(h(ih_com))*theta(ip_com)*eta(isf_com)
            cons_com = (1d0+r)*a(ia_com) + 0.5d0*pen(ij_com) + wage_f - a_plus
        endif
        ...
        if(ism_com > 0 .and. isf_com > 0)then
            ...
            foc = cons_com/sqrt(1d0+dble(nchild(ij_com)))**(1d0-gamma) - tomorrow
        endif
    ...
    function margu(cons, ij, ism, isf)

        implicit none
        real*8, intent(in) :: cons
        integer, intent(in) :: ij, ism, isf
        real*8 :: margu

        if(ism > 0 .and. isf > 0)then
            margu = cons**(-1d0/gamma)/sqrt(2d0+dble(nchild(ij)))**egam
        else
            margu = cons**(-1d0/gamma)/sqrt(1d0+dble(nchild(ij)))**egam
        endif
    ...
    function valuefunc(a_plus, cons, lpart, ij, ih, ip, ism, isf)
        ...
        if(ism > 0 .and. isf > 0)then
            ...
            valuefunc = (c_help/sqrt(1d0+dble(nchild(ij))))**egam/egam &
                            - nu*dble(lpart) + beta*psi(ij+1)*valuefunc
        endif
