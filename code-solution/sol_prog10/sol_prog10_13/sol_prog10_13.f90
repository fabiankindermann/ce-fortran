!##############################################################################
! PROGRAM FemaleParticipationSurvival
!
! ## emale Labor Force Participation with independent survival probabilities
!
...
contains
    ...
    subroutine initialize
        ...
        call discretize_AR(rho, 0d0, sigma_eps, eta(1:NS), pi)
        eta(1:NS) = exp(eta(1:NS))
        eta(0) = 0d0
    ...
    subroutine solve_household()
        ...
        do ia = 0, NA
            aplus(JJ, ia, :, :, :, :) = 0d0
            c(JJ, ia, :, :, 1:NS, 1:NS) = (1d0+r)*a(ia) + pen(JJ)
            c(JJ, ia, :, :, 0, 1:NS) = (1d0+r)*a(ia) + 0.5d0*pen(JJ)
            c(JJ, ia, :, :, 1:NS, 0) = (1d0+r)*a(ia) + 0.5d0*pen(JJ)
            l(JJ, ia, :, :, :, :) = 0d0
            V(JJ, ia, :, :, 1:NS, 1:NS) = valuefunc(0d0, c(JJ, ia, 0, 1, 1, 1), 0, JJ, 0, 1, 1, 1)
            V(JJ, ia, :, :, 0, 1:NS) = valuefunc(0d0, c(JJ, ia, 0, 1, 0, 1), 0, JJ, 0, 1, 0, 1)
            V(JJ, ia, :, :, 1:NS, 0) = valuefunc(0d0, c(JJ, ia, 0, 1, 1, 0), 0, JJ, 0, 1, 1, 0)
        enddo
        ...
        do ij = JJ-1, 1, -1
            ...
            do ia = 0, NA
                            ...
                            do isf = 0, is_max
                                ...
                                do il = 0, il_max
                                    ...
                                    if(x_in < 0d0)then
                                        ...
                                        if(ism > 0 .and. isf > 0)then
                                            wage_f = w_f*dble(il)*(exp(h(ih))*theta(ip)*eta(isf) - pchild(ij))
                                            cons_com = (1d0+r)*a(ia) + pen(ij) + wage_f + wage_m
                                        elseif(isf == 0)then
                                            cons_com = (1d0+r)*a(ia) + 0.5d0*pen(ij) + wage_m
                                        elseif(ism == 0)then
                                            wage_f = w_f*dble(il)*(exp(h(ih))*theta(ip)*eta(isf))
                                            cons_com = (1d0+r)*a(ia) + 0.5d0*pen(ij) + wage_f
                                        endif
                                    endif

                                    ! write screen output in case of a problem
                                    if(check)write(*,'(a, 6i4)')'ERROR IN ROOTFINDING2 : ', ij, ia, ih, ip, ism, isf
                ...
                if(ij >= JR)then
                    aplus(ij, ia, :, :, 1:NS, 1:NS) = aplus(ij, ia, 0, 1, 1, 1)
                    c(ij, ia, :, :, 1:NS, 1:NS) = c(ij, ia, 0, 1, 1, 1)
                    l(ij, ia, :, :, 1:NS, 1:NS) = l(ij, ia, 0, 1, 1, 1)
                    V(ij, ia, :, :, 1:NS, 1:NS) = V(ij, ia, 0, 1, 1, 1)

                    aplus(ij, ia, :, :, 0, 1:NS) = aplus(ij, ia, 0, 1, 0, 1)
                    c(ij, ia, :, :, 0, 1:NS) = c(ij, ia, 0, 1, 0, 1)
                    l(ij, ia, :, :, 0, 1:NS) = l(ij, ia, 0, 1, 0, 1)
                    V(ij, ia, :, :, 0, 1:NS) = V(ij, ia, 0, 1, 0, 1)

                    aplus(ij, ia, :, :, 1:NS, 0) = aplus(ij, ia, 0, 1, 1, 0)
                    c(ij, ia, :, :, 1:NS, 0) = c(ij, ia, 0, 1, 1, 0)
                    l(ij, ia, :, :, 1:NS, 0) = l(ij, ia, 0, 1, 1, 0)
                    V(ij, ia, :, :, 1:NS, 0) = V(ij, ia, 0, 1, 1, 0)
                endif
            enddo
                    ...
                    do ism = 0, is_max
                        do isf = 0, is_max
                            ...
                            do il = 0, il_max
                                ...
                                ! the couple is currently alive
                                if(ism > 0 .and. isf > 0)then

                                    ! husband dies in the next period
                                    do isf_p = 1, NS

                                        ! right hand side of first order condition
                                        chelp = varphi      *c(ij, ia, ihl, ip, 0, isf_p) + &
                                                (1d0-varphi)*c(ij, ia, ihr, ip, 0, isf_p)
                                        chelp = max(chelp, 1d-10)
                                        RHS(ia, ih, ip, ism, isf, il) = RHS(ia, ih, ip, ism, isf, il) + &
                                                (1d0-psi(ij))*pi(isf, isf_p)*margu(chelp, ij, 0, isf_p)

                                        ! expected value function
                                        Vhelp = varphi      *(egam*V(ij, ia, ihl, ip, 0, isf_p))**(1d0/egam) + &
                                                (1d0-varphi)*(egam*V(ij, ia, ihr, ip, 0, isf_p))**(1d0/egam)
                                        Vhelp = max(Vhelp, 1d-10)**egam/egam
                                        EV(ia, ih, ip, ism, isf, il) = EV(ia, ih, ip, ism, isf, il) + &
                                                (1d0-psi(ij))*pi(isf, isf_p)*Vhelp
                                    enddo

                                    ! wife dies in the next period
                                    do ism_p = 1, NS

                                        ! right hand side of first order condition
                                        chelp = varphi      *c(ij, ia, ihl, ip, ism_p, 0) + &
                                                (1d0-varphi)*c(ij, ia, ihr, ip, ism_p, 0)
                                        chelp = max(chelp, 1d-10)
                                        RHS(ia, ih, ip, ism, isf, il) = RHS(ia, ih, ip, ism, isf, il) + &
                                                (1d0-psi(ij))*pi(ism, ism_p)*margu(chelp, ij, ism_p, 0)

                                        ! expected value function
                                        Vhelp = varphi      *(egam*V(ij, ia, ihl, ip, ism_p, 0))**(1d0/egam) + &
                                                (1d0-varphi)*(egam*V(ij, ia, ihr, ip, ism_p, 0))**(1d0/egam)
                                        Vhelp = max(Vhelp, 1d-10)**egam/egam
                                        EV(ia, ih, ip, ism, isf, il) = EV(ia, ih, ip, ism, isf, il) + &
                                                                       (1d0-psi(ij))*pi(ism, ism_p)*Vhelp
                                    enddo

                                    ! both survive in the next period
                                    do ism_p = 1, NS
                                        do isf_p = 1, NS
                                            ...
                                            chelp = max(chelp, 1d-10)
                                            RHS(ia, ih, ip, ism, isf, il) = RHS(ia, ih, ip, ism, isf, il) + &
                                                    psi(ij)*pi(isf, isf_p)*pi(ism, ism_p)*margu(chelp, ij, ism_p, isf_p)
                                            ...
                                            Vhelp = max(Vhelp, 1d-10)**egam/egam
                                            EV(ia, ih, ip, ism, isf, il) = EV(ia, ih, ip, ism, isf, il) + &
                                                    psi(ij)*pi(isf, isf_p)*pi(ism, ism_p)*Vhelp
                                        enddo
                                    enddo

                                ! only the wife is currently alive
                                elseif(ism == 0 .and. isf > 0)then
                                    ...
                                    do isf_p = 1, NS
                                        ...
                                        chelp = varphi      *c(ij, ia, ihl, ip, 0, isf_p) + &
                                                (1d0-varphi)*c(ij, ia, ihr, ip, 0, isf_p)
                                        chelp = max(chelp, 1d-10)
                                        RHS(ia, ih, ip, 0, isf, il) = RHS(ia, ih, ip, 0, isf, il) + &
                                                pi(isf, isf_p)*margu(chelp, ij, 0, isf_p)

                                        ! expected value function
                                        Vhelp = varphi      *(egam*V(ij, ia, ihl, ip, 0, isf_p))**(1d0/egam) + &
                                                (1d0-varphi)*(egam*V(ij, ia, ihr, ip, 0, isf_p))**(1d0/egam)
                                        Vhelp = max(Vhelp, 1d-10)**egam/egam
                                        EV(ia, ih, ip, 0, isf, il) = EV(ia, ih, ip, 0, isf, il) + &
                                                                     pi(isf, isf_p)*Vhelp
                                    enddo

                                ! only the husband is currently alive
                                elseif(isf == 0 .and. ism > 0)then
                                    ...
                                    do ism_p = 1, NS
                                        ...
                                        chelp = varphi      *c(ij, ia, ihl, ip, ism_p, 0) + &
                                                (1d0-varphi)*c(ij, ia, ihr, ip, ism_p, 0)
                                        chelp = max(chelp, 1d-10)
                                        RHS(ia, ih, ip, ism, 0, il) = RHS(ia, ih, ip, ism, 0, il) + &
                                                 pi(ism, ism_p)*margu(chelp, ij, ism_p, 0)

                                        ! expected value function
                                        Vhelp = varphi*(egam*V(ij, ia, ihl, ip, ism_p, 0))**(1d0/egam) + &
                                                (1d0-varphi)*(egam*V(ij, ia, ihr, ip, ism_p, 0))**(1d0/egam)
                                        Vhelp = max(Vhelp, 1d-10)**egam/egam
                                        EV(ia, ih, ip, ism, 0, il) = EV(ia, ih, ip, ism, 0, il) + &
                                                                     pi(ism, ism_p)*Vhelp
                                    enddo
                                endif
                                if(ism > 0 .or. isf > 0)then
                                    RHS(ia, ih, ip, ism, isf, il) = ((1d0+r)*beta*psi(ij)* &
                                                                     RHS(ia, ih, ip, ism, isf, il))**(-gamma)
                        ...
                        do ism = 0, NS
                            do isf = 0, NS
                                ...
                                if(ism > 0 .and. isf > 0)then
                                    ...
                                    ! wife dies in the next period
                                    do ism_p = 1, NS
                                        phi(ij, ial, ihl, ip, ism_p, 0) = &
                                            phi(ij, ial, ihl, ip, ism_p, 0) + &
                                            psi(ij)*(1d0-psi(ij))*pi(ism, ism_p)* &
                                            varphi_a*varphi_h*phi(ij-1, ia, ih, ip, ism, isf)
                                        phi(ij, iar, ihl, ip, ism_p, 0) = &
                                            phi(ij, iar, ihl, ip, ism_p, 0) + &
                                            psi(ij)*(1d0-psi(ij))*pi(ism, ism_p)* &
                                            (1d0-varphi_a)*varphi_h*phi(ij-1, ia, ih, ip, ism, isf)
                                        phi(ij, ial, ihr, ip, ism_p, 0) = &
                                            phi(ij, ial, ihr, ip, ism_p, 0) + &
                                            psi(ij)*(1d0-psi(ij))*pi(ism, ism_p)* &
                                            varphi_a*(1d0-varphi_h)*phi(ij-1, ia, ih, ip, ism, isf)
                                        phi(ij, iar, ihr, ip, ism_p, 0) = &
                                            phi(ij, iar, ihr, ip, ism_p, 0) + &
                                            psi(ij)*(1d0-psi(ij))*pi(ism, ism_p)* &
                                            (1d0-varphi_a)*(1d0-varphi_h)*phi(ij-1, ia, ih, ip, ism, isf)
                                    enddo

                                    ! both survive the next period
                                    do ism_p = 1, NS
                                        do isf_p = 1, NS
                                            phi(ij, ial, ihl, ip, ism_p, isf_p) = &
                                                phi(ij, ial, ihl, ip, ism_p, isf_p) + &
                                                psi(ij)*psi(ij)*pi(ism, ism_p)*pi(isf, isf_p)* &
                                                varphi_a*varphi_h*phi(ij-1, ia, ih, ip, ism, isf)
                                ...
                                elseif(ism == 0 .and. isf > 0)then
                                    ...
                                    do isf_p = 1, NS
                                        phi(ij, ial, ihl, ip, 0, isf_p) = &
                                            phi(ij, ial, ihl, ip, 0, isf_p) + psi(ij)*pi(isf, isf_p)* &
                                            varphi_a*varphi_h*phi(ij-1, ia, ih, ip, 0, isf)
                                        phi(ij, iar, ihl, ip, 0, isf_p) = &
                                            phi(ij, iar, ihl, ip, 0, isf_p) + psi(ij)*pi(isf, isf_p)* &
                                            (1d0-varphi_a)*varphi_h*phi(ij-1, ia, ih, ip, 0, isf)
                                        phi(ij, ial, ihr, ip, 0, isf_p) = &
                                            phi(ij, ial, ihr, ip, 0, isf_p) + psi(ij)*pi(isf, isf_p)* &
                                            varphi_a*(1d0-varphi_h)*phi(ij-1, ia, ih, ip, 0, isf)
                                        phi(ij, iar, ihr, ip, 0, isf_p) = &
                                            phi(ij, iar, ihr, ip, 0, isf_p) + psi(ij)*pi(isf, isf_p)* &
                                            (1d0-varphi_a)*(1d0-varphi_h)*phi(ij-1, ia, ih, ip, 0, isf)
                                    enddo
                                ...
                                elseif(isf == 0 .and. ism > 0)then
                                    ...
                                    do ism_p = 1, NS
                                        phi(ij, ial, ihl, ip, ism_p, 0) = &
                                            phi(ij, ial, ihl, ip, ism_p, 0) + psi(ij)*pi(ism, ism_p)* &
                                            varphi_a*varphi_h*phi(ij-1, ia, ih, ip, ism, 0)
                                        phi(ij, iar, ihl, ip, ism_p, 0) = &
                                            phi(ij, iar, ihl, ip, ism_p, 0) + psi(ij)*pi(ism, ism_p)* &
                                            (1d0-varphi_a)*varphi_h*phi(ij-1, ia, ih, ip, ism, 0)
                                        phi(ij, ial, ihr, ip, ism_p, 0) = &
                                            phi(ij, ial, ihr, ip, ism_p, 0) + psi(ij)*pi(ism, ism_p)* &
                                            varphi_a*(1d0-varphi_h)*phi(ij-1, ia, ih, ip, ism, 0)
                                        phi(ij, iar, ihr, ip, ism_p, 0) = &
                                            phi(ij, iar, ihr, ip, ism_p, 0) + psi(ij)*pi(ism, ism_p)* &
                                            (1d0-varphi_a)*(1d0-varphi_h)*phi(ij-1, ia, ih, ip, ism, 0)
                                    enddo
    ...
    subroutine output()
        ...
        write(21,'(a,a)')' IJ      CONS      FLFP    HUMCAP    WAGE_F    WAGE_M  EARNINGS', &
            '      PENS    ASSETS     CV(C)     CV(L)     VALUE     IAMAX'
        do ij = 1, JJ
