!##############################################################################
! PROGRAM PortfolioChoiceqShares
!
...
contains
    ...
    subroutine calculate_quantiles()
        ...
        real*8 :: omega_min((NA+1)*(NE+1)), omega_min_cdist((NA+1)*(NE+1))
        real*8 :: omega_cur((NA+1)*(NE+1)), omega_cur_cdist((NA+1)*(NE+1))
        ...
        do ij = 2, JJ
            ...
            ! copy portfolio choice into one-dimensional array
            ic = 1
            do ie = 0, NE
                do ia = 0, NA
                    if(phi_a(ij-1, ia)*phi_e(ij, ie) > 1d-12)then
                        ...
                        if(omega_plus(ij-1, ia) > stocks_min)then
                            omega_min(ic) = phi_a(ij-1, ia)*phi_e(ij, ie)
                        endif
                        omega_cur(ic) = omega_plus(ij-1, ia)
                        ic = ic + 1
                    endif
                enddo
            enddo
            ...
            ! fraction that invests more than 50% of wealth in stocks
        ...
        call execplot(xlabel='Age j', ylabel='Frac. with Stock Holdings > 50%')
    ...
    subroutine output()
        ...
        write(21,'(a,a)') ' IJ      CONS    INCOME    ASSETS', &
            '     OMEGA     CV(C)     CV(L)     CV(A)     CV(O)     IXMAX     IAMAX'
        do ij = 1, JJ
