...
!
! ## Female labor force participation with separate pensions
!
...
!
!##############################################################################
include "sol_prog10_14m.f90"

program FemaleParticipationPension
...
contains
    ...
    subroutine initialize
        ...
        ! old-age transfers husband
        pen_m = 0d0
        pen_m(JR:JJ) = 0.4d0*sum(eff)/dble(JR-1)
        ...
        ! old age transfers wife
        pen_f = 0d0
        do ih = 0, NH
            pen_f(JR:JJ, ih) = chi*w_f*h(ih)
        enddo
    ...
    subroutine solve_household()
        ...
        implicit none
        integer :: ij, ia, ih, ip, ip_max, ism, isf, is_max, il, il_max
        ...
        do ia = 0, NA
            do ih = 0, NH
                aplus(JJ, ia, ih, :, :, :) = 0d0
                c(JJ, ia, ih, :, :, :) = (1d0+r)*a(ia) + pen_m(JJ) + pen_f(JJ, ih)
                l(JJ, ia, ih, :, :, :) = 0d0
                V(JJ, ia, ih, :, :, :) = valuefunc(0d0, c(JJ, ia, ih, 1, 1, 1), 0, JJ, ih, 1, 1, 1)
            enddo
        enddo
        ...
        do ij = JJ-1, 1, -1
            ...
            do ia = 0, NA
                do ih = 0, NH
                    ...
                    if(ij >= JR .and. ia == 0 .and. pen_m(ij) <= 1d-10 .and. &
                         pen_f(ij, ih) <= 1d-10)then
                        aplus(ij, ia, ih, :, :, :) = 0d0
                        c(ij, ia, ih, :, :, :) = 0d0
                        l(ij, ia, ih, :, :, :) = 0d0
                        V(ij, ia, ih, :, :, :) = valuefunc(0d0, 0d0, 0, ij, 0, 1, 1, 1)
                        cycle
                    endif
                            ...
                            do isf = 1, is_max
                                ...
                                do il = 0, il_max
                                    ...
                                    if(x_in < 0d0)then
                                        ...
                                        cons_com = (1d0+r)*a(ia) + pen_m(ij) + pen_f(ij, ih) + wage_f + wage_m
                                    endif
                    ...
                    if(ij >= JR)then
                        aplus(ij, ia, ih, :, :, :) = aplus(ij, ia, ih, 1, 1, 1)
                        c(ij, ia, ih, :, :, :) = c(ij, ia, ih, 1, 1, 1)
                        l(ij, ia, ih, :, :, :) = l(ij, ia, ih, 1, 1, 1)
                        V(ij, ia, ih, :, :, :) = V(ij, ia, ih, 1, 1, 1)
                    endif
                enddo
            enddo
    ...
    subroutine output()
        ...
        write(21,'(a,a)')' IJ      CONS      FLFP    HUMCAP    WAGE_F    WAGE_M  EARNINGS', &
            '      PENSM   PENSF  ASSETS     CV(C)     CV(L)     VALUE     IAMAX'
        do ij = 1, JJ
            write(21, '(i3,12f10.3,i10)')ij, c_coh(ij), l_coh(ij), h_coh(ij), &
                yf_coh(ij), ym_coh(ij), yf_coh(ij)+ym_coh(ij), pen_m(ij), sum(pen_f(ij, :))/(NH+1), a_coh(ij), &
        ...
        call plot(dble(ages), ym_coh+yf_coh+pen_m, legend='Total Earnings')
