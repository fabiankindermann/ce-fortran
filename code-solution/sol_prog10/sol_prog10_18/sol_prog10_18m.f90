...
module globals
    ...
    real*8 :: w, eff(JJ), pen(JJ, NO), psi(JJ+1)

    ! cohort aggregate variables
    real*8 :: c_coh(JJ, NO), y_coh(JJ, NO), a_coh(JJ, NO), o_coh(JJ, NO)
    real*8 :: cv_c(JJ, NO), cv_y(JJ, NO), cv_a(JJ, NO), cv_o(JJ, NO), frac_phi(JJ, NO)

    ! individual variables
    real*8 :: X(0:NX), a(0:NA)
    real*8 :: c(JJ, 0:NX, NO), a_plus(JJ, 0:NX, NO), V(JJ, 0:NX, NO) = 0d0
    real*8 :: omega_plus(JJ, 0:NA, NO), Q(JJ, 0:NA, NO)
    real*8 :: phi_X(JJ, 0:NX, NO), phi_a(JJ, 0:NA, NO)
    real*8 :: eta(JJ, 0:NE), phi_e(JJ, 0:NE)

    ! numerical variables
    real*8 :: RHS(JJ, 0:NA, NO)
    integer :: ij_com, ix_com, ia_com, io_com
    real*8 :: cons_com
...
contains
    ...
    function foc_cons(x_in)
        ...
        tomorrow = varphi*RHS(ij_com, ial, io_com) + (1d0-varphi)*RHS(ij_com, iar, io_com)
    ...
    function foc_port(p)
        ...
        if(ij_com+1 >= JR)then
            do isr = 1, NSR
                ...
                X_p = R_port*a(ia_com) + pen(ij_com+1, io_com)
                ...
                c_p = varphi      *c(ij_com+1, ixl, io_com) + &
                      (1d0-varphi)*c(ij_com+1, ixr, io_com)
                c_p = max(c_p, 1d-10)
                ...
                do iw = 1, NW
                    do isr = 1, NSR
                        ...
                        if(io_p == 1)then
                            ...
                            earnings = w*eff(ij_com+1)*chi(io_p)
                        endif
                        ...
                        ! get distributional weight
                        dist = pi(io_com, io_p)*dist_zeta(iw)*dist_epsvtheta(isr)

                        ! calculate the RHS of the FOC
                        c_p = varphi     *c(ij_com+1, ixl, io_p) + &
                             (1d0-varphi)*c(ij_com+1, ixr, io_p)
                        c_p = max(c_p, 1d-10)
    ...
    function valuefunc(a_plus, cons, ij, io)

        implicit none
        integer, intent(in) :: ij, io
        ...
        valuefunc = 0d0
        if(ij < JJ)then
            valuefunc = max(varphi*Q(ij, ial, io) + (1d0-varphi)*Q(ij, iar, io), 1d-10)**egam/egam
        endif
