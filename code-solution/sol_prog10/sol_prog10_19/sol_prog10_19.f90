!##############################################################################
! PROGRAM PortfolioChoiceLabor
!
! ## Portfolio choice with endogenous labor supply (Gomes et al. 2008, AER)
!
...
contains
    ...
    subroutine initialize
        ...
        ! old-age transfers
        pen = 0d0
        pen(JR:JJ) = kappa*w*eff(JR-1)*0.33d0
    ...
    subroutine solve_household()
        ...
        do ix = 0, NX
            ...
            c(JJ, ix, :) = X(ix)
            l(JJ, ix, :) = 0d0
            V(JJ, ix, :) = valuefunc(0d0, c(JJ, ix, 1), 0d0, JJ)
        enddo
        ...
        do ij = JJ-1, 1, -1
            ...
            if(ij >= JR)then
                do ix = 0, NX
                    call solve_consumption(ij, ix, 1)

                    a_plus(ij, ix, :) = a_plus(ij, ix, 1)
                    c(ij, ix, :) = c(ij, ix, 1)
                    l(ij, ix, :) = l(ij, ix, 1)
                    V(ij, ix, :) = V(ij, ix, 1)
                enddo
                ...
                do ix = 0, NX
                    do iw = 1, NW
                        call solve_consumption(ij, ix, iw)
                    enddo
                enddo
            endif
    ...
    subroutine solve_consumption(ij, ix, iw)

        implicit none
        integer, intent(in) :: ij, ix, iw
        real*8 :: x_in, wage
        logical :: check

        ! determine decision for zero cash-on-hand
        if(X(ix) < 1d-10 .and. ij >= JR)then
            a_plus(ij, ix, iw) = 0d0
            c(ij, ix, iw) = 0d0
            V(ij, ix, iw) = valuefunc(0d0, 0d0, 0d0, ij)
            return
        endif
        ...
        if(x_in < 0d0)then
            ...
            if(ij_com < JR)then
                lab_com = min(max(nu + (1d0-nu)/wage*X(ix_com), 0d0), 1d0-1d-10)
            else
                lab_com = 0d0
            endif
            cons_com = max(X(ix_com) + wage*lab_com, 1d-10)
        endif

        ! copy decisions
        a_plus(ij, ix, iw) = x_in
        c(ij, ix, iw) = cons_com
        l(ij, ix, iw) = lab_com
        V(ij, ix, iw) = valuefunc(x_in, cons_com, lab_com, ij)
    ...
    subroutine interpolate(ij)
        ...
        integer :: ia, iw, isr
        real*8 :: X_p, c_p, l_p, varphi, dist, EV, R_port
        integer :: ixl, ixr
        ...
        do ia = 0, NA
            ...
            if(ij >= JR-1)then
                ...
                do isr = 1, NSR
                    ...
                    c_p = max(varphi*c(ij+1, ixl, 1) + (1d0-varphi)*c(ij+1, ixr, 1), 1d-10)

                    ! get tomorrow's value function
                    EV = varphi      *(egam*V(ij + 1, ixl, 1))**(1d0/egam) + &
                         (1d0-varphi)*(egam*V(ij + 1, ixr, 1))**(1d0/egam)

                    ! get RHS of foc and Q
                    RHS(ij, ia) = RHS(ij, ia) + dist*R_port*margu(c_p, 0d0)
            ...
            ! agent is working
                ...
                do iw = 1, NW
                    do isr = 1, NSR
                        ...
                        ! get future consumption and labour value
                        c_p = max(varphi*c(ij+1, ixl, iw) + (1d0-varphi)*c(ij+1, ixr, iw), 1d-10)
                        l_p = min(max(varphi*l(ij+1, ixl, iw) + (1d0-varphi)*l(ij+1, ixr, iw), 0d0), 1d0-1d-10)

                        ! get tomorrow's value function
                        EV = varphi      *(egam*V(ij+1, ixl, iw))**(1d0/egam) + &
                             (1d0-varphi)*(egam*V(ij+1, ixr, iw))**(1d0/egam)

                        ! get RHS of foc and Q
                        RHS(ij, ia) = RHS(ij, ia) + dist*R_port*margu(eps(isr)*c_p, l_p)
                        Q(ij, ia)   = Q(ij, ia) + dist*(eps(isr)**nu*EV)**egam/egam
                    enddo
                enddo
            endif
    ...
    subroutine get_distribution_X(ij)
        ...
        if(ij == 1)then
            ...
            ! get initial distribution at age 1 of cash-on-hand (X = 0)
            do iw = 1, NW
                phi_X(1, 0, iw) = phi_X(1, 0, iw) + dist_zeta(iw)
            enddo
        ...
        elseif(ij <= JR-1)then
            ...
            do ia = 0, NA
                ...
                do iw = 1, NW
                    do isr = 1, NSR
                        ...
                        phi_X(ij, ixl, iw) = phi_X(ij, ixl, iw) + dist*varphi*phi_a(ij-1, ia)
                        phi_X(ij, ixr, iw) = phi_X(ij, ixr, iw) + dist*(1d0-varphi)*phi_a(ij-1, ia)
                    enddo
                enddo
            enddo
            ...
            do ia = 0, NA
                ...
                do isr = 1, NSR
                    ...
                    phi_X(ij, ixl, 1) = phi_X(ij, ixl, 1) + dist*varphi*phi_a(ij-1, ia)
                    phi_X(ij, ixr, 1) = phi_X(ij, ixr, 1) + dist*(1d0-varphi)*phi_a(ij-1, ia)
                enddo
            enddo
        endif
    ...
    subroutine get_distribution_a(ij)
        ...
        do ix = 0, NX
            do iw = 1, NW
                ...
                call linint_Grow(a_plus(ij, ix, iw), a_l, a_u, a_grow, NA, ial, iar, varphi)
                ...
                phi_a(ij, ial) = phi_a(ij, ial) + varphi*phi_X(ij, ix, iw)
                phi_a(ij, iar) = phi_a(ij, iar) + (1d0-varphi)*phi_X(ij, ix, iw)
            enddo
        enddo
    ...
    subroutine aggregation()
        ...
        real*8 :: var_c(JJ), var_a(JJ), var_l(JJ), var_y(JJ), var_o(JJ)
        ...
        if(analytical)then
            ...
            do ij = 1, JJ
                ...
                do ix = 0, NX
                    do iw = 1, NW
                        c_coh(ij) = c_coh(ij) + c(ij, ix, iw)*phi_X(ij, ix, iw)
                        l_coh(ij) = l_coh(ij) + l(ij, ix, iw)*phi_X(ij, ix, iw)
                        y_coh(ij) = y_coh(ij) + (w*eff(ij)*zeta(iw)*l(ij, ix, iw) + pen(ij))*phi_X(ij, ix, iw)
                    enddo
                enddo
            ...
            do ij = 1, JJ
                do ie = 0, NE
                    ...
                    do ix = 0, NX
                        do iw = 1, NW
                            c_coh(ij) = c_coh(ij) + c(ij, ix, iw)*eta(ij, ie)*phi_X(ij, ix, iw)*phi_e(ij, ie)
                            l_coh(ij) = l_coh(ij) + l(ij, ix, iw)*phi_X(ij, ix, iw)*phi_e(ij, ie)
                            y_coh(ij) = y_coh(ij) + (w*eff(ij)*zeta(iw)*l(ij, ix, iw) + pen(ij))*eta(ij, ie) &
                                        *phi_X(ij, ix, iw)*phi_e(ij, ie)
                        enddo
                    enddo
        ...
        if(analytical)then
            ...
            do ij = 1, JJ
                ...
                do ix = 0, NX
                    do iw = 1, NW
                        var_c(ij) = var_c(ij) + c(ij, ix, iw)**2*phi_X(ij, ix, iw)
                        var_l(ij) = var_l(ij) + l(ij, ix, iw)**2*phi_X(ij, ix, iw)
                        var_y(ij) = var_y(ij) + ((w*eff(ij)*zeta(iw)*l(ij, ix, iw) + pen(ij)))**2*phi_X(ij, ix, iw)
                    enddo
                enddo
            ...
            do ij = 1, JJ
                do ie = 0, NE
                    do ix = 0, NX
                        do iw = 1, NW
                            var_c(ij) = var_c(ij) + (c(ij, ix, iw)*eta(ij, ie))**2*phi_X(ij, ix, iw)*phi_e(ij, ie)
                            var_y(ij) = var_y(ij) + l(ij, ix, iw)**2*phi_X(ij, ix, iw)*phi_e(ij, ie)
                            var_y(ij) = var_y(ij) + ((w*eff(ij)*zeta(iw)*l(ij, ix, iw) + pen(ij))*eta(ij, ie))**2 &
                                        *phi_X(ij, ix, iw)*phi_e(ij, ie)
                        enddo
                    enddo
        ...
        cv_l = sqrt(max(var_l, 0d0))/max(l_coh, 1d-10)
    ...
    subroutine output()
        ...
        write(21,'(a,a)')' IJ      CONS     LABOR    INCOME    ASSETS', &
            '     OMEGA     CV(C)     CV(L)     CV(Y)     CV(A)     CV(O)     IXMAX     IAMAX'
        do ij = 1, JJ
            write(21,'(i3,10f10.3,2i10)')ages(ij), c_coh(ij), l_coh(ij), y_coh(ij), a_coh(ij), o_coh(ij), &
                cv_c(ij), cv_l(ij), cv_y(ij), cv_a(ij), cv_o(ij), ixmax(ij), iamax(ij)
        enddo
        write(21,'(a/)') '--------------------------------------------------------------------'

        ! plot output
        call plot(dble(ages), c_coh, legend='Consumption (Mean)')
        call plot(dble(ages), l_coh, legend='Labor Hours (Mean)')
    ...
    subroutine check_grid_X(ixmax)
        ...
        do ij = 1, JJ
            ...
            do ix = 0, NX
                do iw = 1, NW
                    if(phi_X(ij, ix, iw) > 1d-8)ixmax(ij) = ix
                enddo
            enddo
        enddo
