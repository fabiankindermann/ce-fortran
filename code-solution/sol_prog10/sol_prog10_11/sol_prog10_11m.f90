...
module globals
    ...
    real*8, parameter :: nu_1 = 0.04d0
    real*8, parameter :: nu_2 = 0.12d0
...
contains
    ...
    function foc(x_in)
        ...
        foc = cons_com/sqrt(2d0+dble(nchild(ij_com)))**(1d0-gamma)* &
                    (exp(nu_1*dble(il_com)))**(-gamma) - tomorrow

    end function


    ! calculates marginal utility of consumption
    function margu(cons, il, ij)

        implicit none
        real*8, intent(in) :: cons
        integer, intent(in) :: il, ij
        real*8 :: margu

        margu = cons**(-1d0/gamma)/sqrt(2d0+dble(nchild(ij)))**egam*exp(nu_1*dble(il))
    ...
    function valuefunc(a_plus, cons, lpart, ij, ih, ip, ism, isf)
        ...
        valuefunc = (c_help/sqrt(2d0+dble(nchild(ij))))**egam/egam*exp(nu_1*dble(lpart)) &
                        - nu_2*dble(lpart) + beta*psi(ij+1)*valuefunc
