...
! PROGRAM RamseyLog
!
! ## Ramsey model with log-utility using policy and value function iteration
!
...
program RamseyLog
    ...
    do iter = 1, itermax
        ...
        do ik = 0, NK
            ...
            call fminsearch(x_in, fret, k_l, min(k(ik)**alpha, k_u), utility)
        ...
        ! interpolate coefficients
        call spline_interp(V_new, coeff_V)
...
contains
    ...
    subroutine output()
        ...
        theta = 1d0/beta - 1d0
        k_stat = (alpha/(1d0+theta))**(1d0/(1d0-alpha))
        y_stat = k_stat**alpha
        c_stat = y_stat - k_stat
        ...
        y_t(0) = k_t(0)**alpha
        do it = 1, TT
            k_t(it) = k_t(it-1)**alpha - c_t(it-1)
        ...
        k_t(0) = k0
        c_t(0) = k_t(0)**alpha*(1d0 - beta*alpha)
        do it = 1, TT
            k_t(it) = k_t(it-1)**alpha - c_t(it-1)
            c_t(it) = k_t(it)**alpha*(1d0 - beta*alpha)
        enddo
        ...
        call plot((/(dble(it),it=0,TT)/), c_t+k_t, legend='Cons. + repl.Inv')
