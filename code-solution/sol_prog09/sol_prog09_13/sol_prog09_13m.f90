...
module globals
    ...
    real*8 :: a(0:NA), c(0:NA, NS)

    ! variables to determine policy function
    real*8 :: a_endog(-1:NA, NS), c_endog(-1:NA, NS), c_new(0:NA, NS)
    real*8 :: RHS(0:NA, NS)

    ! variables to determine the steady state distribution
    real*8 :: phi(0:NA, NS), phi_new(0:NA, NS)

    ! numerical variables for EER
    real*8 :: a_com
    integer :: is_com
...
contains
    ...
    subroutine solve_household()
        ...
        integer :: ia, is, is_p, iter
        real*8 :: con_lev

        ! do a policy function iteration with endogenous gridpoints
        do iter = 1, itermax
            ...
            do ia = 0, NA
                do is = 1, NS
                    ...
                    c_endog(ia, is) = 0d0
                    do is_p = 1, NS
                        c_endog(ia, is) = c_endog(ia, is) + pi(is, is_p) &
                                          *max(c(ia, is_p), 1d-10)**(-1d0/gamma)
                    enddo
                    c_endog(ia, is) = (beta*(1d0+r)*c_endog(ia, is))**(-gamma)

                    ! check whether any tomorrow's consumption is zero
                    if(any(c(ia, :) < 1d-10))c_endog(ia, is) = 0d0

                    ! get endogenous gridpoint
                    a_endog(ia, is) = (a(ia) + c_endog(ia, is) - w*eta(is))/(1d0+r)
                enddo
            enddo

            ! set the borrowing limit
            do is = 1, NS
                if(a_endog(0, is) >= a_l)then
                    a_endog(-1, is) = a_l
                    c_endog(-1, is) = w*eta(is)
                else
                    a_endog(-1, is) = a_endog(0, is)
                    c_endog(-1, is) = c_endog(0, is)
                endif
            enddo

            ! get the updated values for the consumption function
            do ia = 0, NA
                do is = 1, NS
                    c_new(ia, is) = linint_Gen(a(ia), a_endog(:, is), c_endog(:, is), ia)
                enddo
            enddo
    ...
    function foc(x_in)
        ...
        ! future assets
        aplus = (1d0+r)*a_com + eta(is_com)*w - x_in
    ...
    subroutine interpolate()
        ...
        do is = 1, NS
            do ia = 0, NA
                ...
                RHS(ia, is) = 0d0
                do is_p = 1, NS
                    RHS(ia, is) = RHS(ia, is) + pi(is, is_p)*c(ia, is_p)**(-1d0/gamma)
                enddo
