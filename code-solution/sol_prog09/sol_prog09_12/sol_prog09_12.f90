!##############################################################################
! PROGRAM RBCgovernmentTax
!
! ## RBC model with labor earnings vs. income taxation
!
...
program RBCgovernmentTax
    ...
    ! simulate economy with labor earnings tax
    income_tax = .false.
    tau_y = 0d0
    call getPolicy()
    call output()
    ...
    ! simulate economy with income tax
    income_tax = .true.
    tau_w = 0d0
    call getPolicy()
    call output()
...
contains
        ...
        do iter = 1, itermax
            ...
            do ik = 0, NK
                do is = 1, NS
                    ...
                    l(ik, is) = x_in
                    tau_w(ik, is) = tau_w_com
                    tau_y(ik, is) = tau_y_com
                    c_new(ik, is) = nu/(1d0-nu)*(1d0-x_in)*(1d0-tau_w_com-tau_y_com)* &
        ...
        do is = 1, NS
            do ik = 0, NK
                ...
                do is_p = 1, NS
                    ...
                    RHS(ik, is) = RHS(ik, is) + pi(is, is_p)*marg_util &
                                  *(1d0+(1d0-tau_y(ik, is_p))*(exp(eta(is_p)) &
                                  *alpha*(k(ik)/l(ik, is_p))**(alpha-1d0)-delta))
                enddo
    ...
    subroutine output()
        ...
        write(*,'(a)')'                        c      g      i      k      l   tauw   tauy      y'
        write(*,'(a,7f7.2)')'Mean:             ', (/E_c, E_g, E_i, E_k/)/E_y*100d0, E_l*100d0, &
                                                  E_tauw*100d0, E_tauy*100d0

        write(*,'(a,8f7.2)')'CV (in %):        ', (/CV_c, CV_g, CV_i, CV_k, CV_l, CV_tauw, CV_tauy, CV_y/)*100d0
        write(*,'(a,7f7.2)')'Cor. with output: ', Cor_c, Cor_g, Cor_i, Cor_k, Cor_l, Cor_tauw, Cor_tauy
        write(*, '(/a,f6.1,a,f6.1/)')'Labor earnings tax rate from ', minval(tau_w)*100, ' to ', maxval(tau_w)*100
    ...
    subroutine simulate_economy()
        ...
        if(income_tax)then
            tauy_t(0) = g_t(0)/(w_t(0)*l_t(0) + r_t(0)*k_t(0))
            tauw_t(0) = 0d0
        else
            tauw_t(0) = g_t(0)/(w_t(0)*l_t(0))
            tauy_t(0) = 0d0
        endif
        do it = 1, TT
            ...
            if(income_tax)then
                tauy_t(it) = g_t(it)/(w_t(it)*l_t(it) + r_t(it)*k_t(it))
                tauw_t(it) = 0d0
            else
                tauw_t(it) = g_t(it)/(w_t(it)*l_t(it))
                tauy_t(it) = 0d0
            endif
        enddo
    ...
    subroutine bc_statistics()
        ...
        E_tauw = sum(tauw_t)/dble(TT+1)
        E_tauy = sum(tauy_t)/dble(TT+1)
        ...
        CV_w = CofVar(w_t)
        CV_y = CofVar(y_t)
        CV_tauw = CofVar(tauw_t)
        CV_tauy = CofVar(tauy_t)
        ...
        Cor_tauw = correlation(tauw_t, y_t)
        Cor_tauy = correlation(tauy_t, y_t)
