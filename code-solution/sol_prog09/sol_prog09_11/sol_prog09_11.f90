!##############################################################################
! PROGRAM RBCgovernmentDistortion
!
! ## RBC model with pro-cyclical vs. constant policy and distortive taxes
!
...
contains
    ...
    ! Calculates policy functions in the RBC model
        ...
        do iter = 1, itermax
            ...
            do ik = 0, NK
                do is = 1, NS
                    ...
                    c_new(ik, is) = nu/(1d0-nu)*(1d0-x_in)*(1d0-tau_w_com)* &
            ...
            do ik = 0, NK
                ...
                do is_p = 1, NS
                    ...
                    RHS(ik, is) = RHS(ik, is) + pi(is, is_p)*marg_util* &
                         (1d0+(exp(eta(is_p))*alpha*(k(ik)/l(ik, is_p))**(alpha-1d0)-delta))
                enddo
    ...
    subroutine output()
        ...
        write(*,'(a)')'                         c       g       i       k       l    tauw       y'
        write(*,'(a,6f8.2)')'Mean:             ', (/E_c, E_g, E_i, E_k/)/E_y*100d0, E_l*100d0, E_tauw*100d0

        write(*,'(a,7f8.2)')'CV (in %):        ', (/CV_c, CV_g, CV_i, CV_k, CV_l, CV_tauw, CV_y/)*100d0
        write(*,'(a,6f8.2)')'Cor. with output: ', Cor_c, Cor_g, Cor_i, Cor_k, Cor_l, Cor_tauw
        write(*, '(/a,f6.1,a,f6.1/)')'Wage tax rate from ', minval(tau_w)*100, ' to ', maxval(tau_w)*100
