...
module globals
    ...
    real*8 :: k0 = 15d0
    real*8 :: c_t(0:TT), l_t(0:TT), k_t(0:TT), eta_t(0:TT), tauw_t(0:TT)
    ...
    real*8 :: k(0:NK), c(0:NK, NS), l(0:NK, NS), tau_w(0:NK, NS)
    ...
    real*8 :: E_c, E_g, E_i, E_k, E_l, E_r, E_w, E_y, E_tauw
    real*8 :: CV_c, CV_g, CV_i, CV_k, CV_l, CV_r, CV_w, CV_y, CV_tauw
    real*8 :: Cor_c, Cor_g, Cor_i, Cor_k, Cor_l, Cor_r, Cor_w, Cor_tauw
...
contains
    ...
    function foc(x_in)
        ...
        ! calculate labor tax and optimal actual consumption level
        tau_w_com = gov/(w_act*lab)
        tau_w_com = min(max(tau_w_com, -1d0), 1d0)
        c_act = nu/(1d0-nu)*(1d0-lab)*(1d0-tau_w_com)*w_act

        ! calculate future capital
        kplus = (1d0-tau_w_com)*w_act*lab + (1d0+r_act)*k_com - c_act
