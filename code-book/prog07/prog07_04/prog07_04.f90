...
contains
    ...
    subroutine output(it, fname)
        ...
        do ij = 1, JJ
            itp = year(it, 1, 2)
            if(ij < JJ)then
                diff = Rn(it)*a(ij, it) + wn(it)*h(ij) + beq(ij, it) + pen(ij, it) &
    ...
    subroutine output_summary(fname)
        ...
        ! headline
        write(fname,'(a,a,a)')'         C      A      K      L     BQ      r', &
        ...
        enddo
        write(fname,'(i3,18f7.2)')0, CC(0), AA(0), KK(0), LL(0), BQ(0), r(0), w(0), &
        ...
        do it = 1, TT
            ...
            write(fname,'(i3,18f7.2)')it, CC(it), AA(it), KK(it), LL(it), BQ(it), r(it), w(it), &
