...
contains
    ...
    function foc(x_in)
        ...
        if(ij_com < JR)available = available + Tr(it_com)
    ...
    function margu(cons, lab, it)
        ...
        margu = nu*(cons**nu*(1d0-lab)**(1d0-nu))**(1d0-1d0/gamma)/(p(it)*cons)
    ...
    function valuefunc(a_plus, cons, lab, ij, ip, is, it)
        ...
        if(ij < JJ)then
            valuefunc = max(varphi*EV(ij+1, ial, ip, is, itp) + &
                (1d0-varphi)*EV(ij+1, iar, ip, is, itp), 1d-10)**(1d0-1d0/gamma)/(1d0-1d0/gamma)
        endif

        ! add todays part and discount
        valuefunc = (c_help**nu*(1d0-l_help)**(1d0-nu))**(1d0-1d0/gamma)/(1d0-1d0/gamma) + beta*valuefunc
